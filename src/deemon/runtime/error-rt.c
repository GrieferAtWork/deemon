...
#include <deemon/object.h>             /* DeeObject_*, Dee_BOUND_ERR, Dee_Decref, Dee_Decref_unlikely, Dee_Incref, Dee_XDecref, Dee_XDecref_unlikely, Dee_XIncref, return_reference, return_reference_ */
...
#include <stdint.h>  /* SIZE_MAX, int32_t, int64_t, uint32_t, uint64_t */
...
(DCALL DeeRT_ErrDivideByZero)(DeeObject *lhs, DeeObject *rhs) {
	...
	return DeeError_ThrowInherited(&result->dbz_base);
err:
	return -1;
}
...
(DCALL DeeRT_ErrIndexOutOfBounds)(DeeObject *seq, size_t index, size_t length) {
	...
	return DeeError_ThrowInherited(&result->ie_base.ke_base);
err:
	return -1;
}
