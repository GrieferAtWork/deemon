...
deepcopy_lookup(DeeThreadObject *__restrict thread_self,
 ...
 for (;; i = DEEPASSOC_HASHNX(i,perturb),DEEPASSOC_HASHPT(perturb)) {
  ...
  if (item->de_old != old_object) { if (!item->de_old) break; continue; }
  if unlikely(Dee_TYPE(item->de_new) != new_type) continue;
  return item->de_new;
 }
 return NULL;
}
...
deepcopy_clear(DeeThreadObject *__restrict thread_self) {
 ...
 struct deep_assoc_entry *begin; size_t mask;
...
PUBLIC uint64_t (DCALL DeeThread_GetTimeMicroSeconds)(void) {
 ...
 if (!performance_freq) {
  ...
  if unlikely(!QueryPerformanceFrequency((LARGE_INTEGER *)&new_freq))
     goto do_tickcount;
  if unlikely(!new_freq) new_freq = 1;
...
#endif
   unsigned int timeout; int error;
...
#endif
#endif
  do state = ATOMIC_READ(iter->t_state);
...
destroy_thread_self(DREF DeeThreadObject *__restrict self) {
 dref_t refcnt; uint16_t old_state;
 ...
 for (;;) {
  ...
  do old_state = ATOMIC_READ(self->t_state);
...
next_interrupt:
 ...
 if ((next = self->t_interrupt.ti_next) != NULL) {
  ...
  if (!interrupt_args) goto throw_main;
...
#endif
{
 ...
 { uint16_t state;
   do state = ATOMIC_READ(self->t_state);
...
set_result:
  ...
  if (current && DeeThreadExit_Check(current)) {
   ...
   if (!DeeError_Current()) goto set_result;
   Dee_Decref(result);
  }
...
DeeThread_Wake(/*Thread*/DeeObject *__restrict self) {
  ...
  *(void **)&ptr = GetProcAddress(GetModuleHandleW(kernel32),"CancelSynchronousIo");
  if (!ptr) *(void **)&ptr = (void *)ITER_DONE;
...
reread_state:
 ...
 if (!me->t_interrupt.ti_intr) {
  ...
  while (last_entry->ti_next) last_entry = last_entry->ti_next;
 ...
 do state = ATOMIC_READ(me->t_state);
...
#error "Invalid configuration"
   ...
   if (timeout_microseconds == (uint64_t)-1) {
    ...
    for (;;) {
     if (ATOMIC_READ(me->t_join)) break;
     if (DeeThread_Sleep(1000)) {
      ATOMIC_FETCHAND(me->t_state,~THREAD_STATE_DETACHING);
      goto err;
     }
     ...
     do {
      if (!Dee_CollectMemory(sizeof(struct except_frame))) {
       while (error_frames) {
        ...
        DeeObject_Free(error_frames->ef_error);
...
DeeThread_GetTid(/*Thread*/DeeObject *__restrict self,
 ...
 ASSERT(pthreadid);
 ASSERT_OBJECT_TYPE(self,&DeeThread_Type);
#if defined(CONFIG_NO_THREADID_INTERNAL) || \
    defined(CONFIG_NO_THREADID)
...
PUBLIC bool (DCALL DeeThread_ClearTls)(void) {
 DeeThreadObject *caller; bool result = false;
 caller = (DeeThreadObject *)thread_tls_get();
 if likely(caller) {
...
thread_doclear(DeeThreadObject *__restrict self) {
 bool result = false; void *tls_data;
 ...
 if (old_objects[2]) Dee_Decref(old_objects[2]),result  = true;
 if (old_objects[1]) Dee_Decref(old_objects[1]),result |= old_objects[1] != Dee_EmptyTuple;
 if (old_objects[0]) Dee_Decref(old_objects[0]),result  = true;
 return result;
}
...
PRIVATE void DCALL err_no_thread_api(void) {
...
thread_timedjoin(DeeObject *__restrict self, size_t argc,
                 DeeObject **__restrict argv) {
 int error; DeeObject *result; uint64_t timeout;
...
thread_callback_set(DeeThreadObject *__restrict self,
 ...
 if (!DeeThread_CheckExact(self)) {
  err_cannot_set_thread_subclass_callback(self,"callback");
  return -1;
 }
restart:
 do {
...
thread_callargs_set(DeeThreadObject *__restrict self,
 ...
 if (!DeeThread_CheckExact(self)) {
  err_cannot_set_thread_subclass_callback(self,"callargs");
  return -1;
 }
restart:
 do {
...
again:
 ...
 if (SleepEx((DWORD)(microseconds/1000),TRUE)) {
  uint64_t now = DeeThread_GetTimeMicroSeconds();
  if (DeeThread_CheckInterrupt())
      goto err;
 ...
 * instruction so long as locals are once again value when
...
thread_collect_traceback(DeeThreadObject *__restrict self,
 ...
 uint16_t count = self->t_execsz; size_t i;
...
DeeThread_Trace(/*Thread*/DeeObject *__restrict self) {
 ...
 if (me != DeeThread_Self()) {
  ...
  for (;;) {
   ...
   DeeTracebackObject *result; struct localheap heap;
...
suspend_me:
   ...
   if (traceback_used > traceback_size) {
    ...
    new_result = (DeeTracebackObject *)DeeGCObject_Realloc(result,offsetof(DeeTracebackObject,tb_frames)+
                                                           traceback_size*sizeof(struct code_frame));
    if unlikely(!new_result) goto err_free_result;
    result = new_result;
    goto suspend_me;
   }
   ...
   if unlikely(heap.lh_req > heap.lh_total) {
    ...
    if unlikely(!new_heap) goto err_free_result;
