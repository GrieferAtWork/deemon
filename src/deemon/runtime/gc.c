...
gc_deps_insert(struct gc_deps *__restrict self,
 ...
 for (;; i = VD_HASHNX(i,perturb),VD_HASHPT(perturb)) {
  ...
  if (!dep->gd_object) break;
  if (dep->gd_object != obj) continue;
  /* It has! */
  if (dep->gd_extern)
      --dep->gd_extern;
  return;
...
do_the_visit:
 ...
 for (;; i = VD_HASHNX(i,perturb),VD_HASHPT(perturb)) {
  ...
  if (!dep->gd_object) break;
  if (dep->gd_object != self) continue;
  /* Found it! */
#ifdef GC_ASSERT_REFERENCE_COUNTS
  ASSERT(dep->gd_extern != 0);
#endif
 ...
 for (; link; link = link->dc_prev,++i) {
  ...
  for (j = 0; j < link->dc_pnum; ++j) {
   if (link->dc_pvec[j].dp_obj != self) continue;
   /* Found a partner! */
...
found_chain_link:
  ...
  if (i != 0) {
   ...
   dst = link->dc_pnum;
   new_count = dst + i;
   new_vec = (struct gc_dep_partner *)Dee_TryRealloc(link->dc_pvec,new_count*
...
gc_trydestroy(struct gc_head *__restrict head,
 ...
 for (;;) {
  ...
  if unlikely(!init_dep->gd_extern) return 0; /* Object is already dead! */
...
#define OPTIMIZE_DEPS_SET() \
 ...
 if (visit.vd_deps.gd_err) goto done;
 ...
 for (;;) {
  ...
  for (i = 0; i < visit.vd_deps.gd_cnt; ++i) {
   ...
   if (prio_min > prio) prio_min = prio;
...
#endif
 {
  if (!visit.vd_deps.gd_vec[i].gd_object) continue;
...
INTERN bool DCALL DeeGC_IsEmptyWithoutDex(void) {
 ...
 for (; iter; iter = iter->gc_next) {
  if (!iter->gc_object.ob_refcnt) continue;
#ifndef CONFIG_NO_DEX
  if (DeeDex_Check(&iter->gc_object)) continue;
#endif
#ifndef CONFIG_NO_THREADS
...
INTERN size_t DCALL DeeExec_KillUserCode(void) {
 struct gc_head *iter; size_t result = 0;
 GCLOCK_ACQUIRE();
#ifdef CONFIG_HAVE_PENDING_GC_OBJECTS
 if (gc_pending_front != NULL) {
collect_restart_with_pending_hint:
