...
#include <deemon/bool.h>            /* DeeBool*, Dee_CONFIG_BOOL_TLS, Dee_False, Dee_True */
       ...
       * By passing `DeeInt_One' now, we can skip those checks later! */
				...
				RAW_TARGET(ASM16_CALLATTR_C_KW) {
...
#ifdef EXEC_SAFE
					{
						...
						if unlikely(!DeeString_CheckExact(attr_name))
							goto err_requires_string;
						call_result = DeeObject_CallAttrKw(new_sp[-1], attr_name, argc, new_sp, kwds_map);
					}
#else /* EXEC_SAFE */
					ASSERT_STRING(CONSTimm);
					call_result = DeeObject_CallAttrKw(new_sp[-1], CONSTimm, argc, new_sp, CONSTimm2);
#endif /* !EXEC_SAFE */
					if unlikely(!call_result)
						HANDLE_EXCEPT();
					while (sp > new_sp)
...
do_supergetattr_rc:
...
#ifdef EXEC_SAFE
					{
						...
						if unlikely(!DeeString_Check(attr_name)) {
...
#undef DEFINE_INPLACE_MATH_OPERATOR
				...
				PREFIX_RAW_TARGET(ASM_ADD_SIMM8) {
					...
					if (prefix_pointer != (DREF DeeObject **)ITER_DONE) {
						if unlikely(!prefix_pointer)
							HANDLE_EXCEPT();
						error = DeeObject_InplaceAddInt8(prefix_pointer, READ_Simm8());
						if unlikely(error)
							HANDLE_EXCEPT();
					} else {
						USING_PREFIX_OBJECT
						...
						if unlikely(!prefix_ob)
							HANDLE_EXCEPT();
						error = DeeObject_InplaceAddInt8(&prefix_ob, READ_Simm8());
						if unlikely(error) {
							Dee_Decref(prefix_ob);
							HANDLE_EXCEPT();
						}
				...
				PREFIX_RAW_TARGET(ASM_SUB_SIMM8) {
					...
					if (prefix_pointer != (DREF DeeObject **)ITER_DONE) {
						if unlikely(!prefix_pointer)
							HANDLE_EXCEPT();
						error = DeeObject_InplaceSubInt8(prefix_pointer, READ_Simm8());
						if unlikely(error)
							HANDLE_EXCEPT();
					} else {
						USING_PREFIX_OBJECT
						...
						if unlikely(!prefix_ob)
							HANDLE_EXCEPT();
						error = DeeObject_InplaceSubInt8(&prefix_ob, READ_Simm8());
						if unlikely(error) {
							Dee_Decref(prefix_ob);
							HANDLE_EXCEPT();
						}
