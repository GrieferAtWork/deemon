...
DeeKwds_NewWithHint(size_t num_items) {
 DREF Kwds *result; size_t init_mask = 1;
...
INTERN DREF Kwds *DCALL kwds_rehash(DREF Kwds *__restrict self) {
 DREF Kwds *result; size_t i,j,perturb;
...
(DCALL DeeKwds_Append)(DREF DeeObject **__restrict pself,
 ...
 dhash_t i,perturb; struct kwds_entry *entry;
 ...
 if unlikely(!entry->ke_name) return -1;
...
kwds_repr(Kwds *__restrict self) {
 ...
 for (i = 0; i <= self->kw_mask; ++i) {
  if (!self->kw_map[i].ke_name) continue;
  if (!is_first && UNICODE_PRINTER_PRINT(&printer,", ") < 0) goto err;
...
#ifndef CONFIG_NO_THREADS
 if (!ATOMIC_READ(self->kmo_argv)) return 0;
#else
 if (!self->kmo_argv) return 0;
#endif
...
clear_argv:
   ...
   do argv = (DREF DeeObject **)Dee_TryMalloc(argc * sizeof(DREF DeeObject *));
   while (unlikely(!argv) && Dee_TryCollectMemory(argc * sizeof(DREF DeeObject *)));
   if unlikely(!argv) goto clear_argv;
   rwlock_write(&me->kmo_lock);
