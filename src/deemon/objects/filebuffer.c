...
DeeFileBuffer_New(DeeObject *__restrict file,
	...
	if unlikely(buffer_init(result, file, mode, size))
		goto err_r;
	DeeFileObject_InitStatic(result, &DeeFileBuffer_Type);
done:
...
/*[[[deemon (print_DeeArg_Unpack from rt.gen.unpack)("_FileBuffer", params: """
	DeeFileObject *fp;
	char const *mode=!Pauto = NULL;
	size_t size = 0
""", docStringPrefix: "buffer");]]]*/
#define buffer__FileBuffer_params "fp:?DFile,mode=!Pauto,size=!0"
	struct {
		DeeFileObject *fp;
		char const *mode;
		size_t size;
	...
	args.mode = NULL;
	args.size = 0;
	if (DeeArg_UnpackStruct(argc, argv, "o|s" UNPuSIZ ":_FileBuffer", &args))
		goto err;
/*[[[end]]]*/
	if (args.mode) {
		char const *mode_iter = args.mode;
		unsigned int i;
		union {
			char chrs[4];
			uint32_t id;
