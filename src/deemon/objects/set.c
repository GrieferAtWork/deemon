    ...
    /* .tp_init = */{
        {
            /* .tp_alloc = */{
                ...
                /* .tp_free      = */NULL,
                {
                    /* .tp_instance_size = */sizeof(DeeInverseSetObject)
                }
            }
        },
...
DeeSet_IsSubSet(DeeObject *__restrict lhs, DeeObject *__restrict rhs) {
 if (DeeInverseSet_CheckExact(lhs)) {
  ...
  return unlikely(result == -2) ? -1 : result < 0 ? 0 : 1;
 }
}
INTERN int DCALL
DeeSet_IsTrueSubSet(DeeObject *__restrict lhs, DeeObject *__restrict rhs) {
 if (DeeInverseSet_CheckExact(lhs)) {
  ...
  dssize_t result; size_t rhs_size;
  result = set_issubset_impl(lhs,rhs);
  if unlikely(result == -2) return -1;
  ...
  if unlikely(rhs_size == (size_t)-1) return -1;
...
DeeSet_IsDisjoint(DeeObject *__restrict lhs, DeeObject *__restrict rhs) {
 DREF DeeObject *iter,*item; int result = 1;
 if (DeeInverseSet_CheckExact(lhs)) {
