...
rehash(DREF Set *__restrict self, size_t old_mask, size_t new_mask) {
 DREF Set *result; size_t i;
 result = ROSET_ALLOC(new_mask);
 if unlikely(!result) goto done;
 for (i = 0; i <= old_mask; ++i) {
  size_t j,perturb; struct roset_item *item;
  if (!self->rs_elem[i].si_key) continue;
...
err_r:
 for (elem_count = 0; elem_count <= mask; ++elem_count) {
  if (!result->rs_elem[elem_count].si_key) continue;
...
DeeRoSet_ContainsString(DeeObject *__restrict self,
 ...
 for (;; i = ROSET_HASHNX(i,perturb),ROSET_HASHPT(perturb)) {
  ...
  if (!item->si_key) break;
  if (item->si_hash != hash) continue;
  if (!DeeString_Check(item->si_key)) continue;
  if (DeeString_SIZE(item->si_key) != key_length) continue;
  if (memcmp(key,DeeString_STR(item->si_key),key_length) != 0) continue;
  /* Found it! */
  return true;
 }
 return false;
...
roset_deepcopy(Set *__restrict self) {
 DREF Set *result; size_t i; int temp;
