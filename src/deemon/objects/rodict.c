...
#endif
  if (item >= end) goto iter_exhausted;
  while (item != end && !item->di_key) ++item;
  if (item == end) {
#ifdef CONFIG_NO_THREADS
   self->di_next = item;
#else
...
PRIVATE struct type_cmp rodictiterator_cmp = {
    /* .tp_hash = */NULL,
    /* .tp_eq   = */(DREF DeeObject *(DCALL *)(DeeObject *__restrict,DeeObject *__restrict))&rodictiterator_eq,
    /* .tp_ne   = */(DREF DeeObject *(DCALL *)(DeeObject *__restrict,DeeObject *__restrict))&rodictiterator_ne,
    /* .tp_lo   = */(DREF DeeObject *(DCALL *)(DeeObject *__restrict,DeeObject *__restrict))&rodictiterator_lo,
    /* .tp_le   = */(DREF DeeObject *(DCALL *)(DeeObject *__restrict,DeeObject *__restrict))&rodictiterator_le,
    /* .tp_gr   = */(DREF DeeObject *(DCALL *)(DeeObject *__restrict,DeeObject *__restrict))&rodictiterator_gr,
    /* .tp_ge   = */(DREF DeeObject *(DCALL *)(DeeObject *__restrict,DeeObject *__restrict))&rodictiterator_ge
};
...
rehash(DREF Dict *__restrict self, size_t old_mask, size_t new_mask) {
 DREF Dict *result; size_t i;
 result = RODICT_ALLOC(new_mask);
 if unlikely(!result) goto done;
 for (i = 0; i < old_mask; ++i) {
  size_t j,perturb; struct rodict_item *item;
  if (!self->rd_elem[i].di_key) continue;
...
DeeRoDict_NewWithHint(size_t num_items) {
 ...
 while (mask <= num_items) mask = (mask << 1)|1;
 mask = (mask << 1)|1;
...
err_r:
 for (elem_count = 0; elem_count <= mask; ++elem_count) {
  if (!result->rd_elem[elem_count].di_key) continue;
...
rodict_contains(Dict *__restrict self,
 ...
 for (;; i = RODICT_HASHNX(i,perturb),RODICT_HASHPT(perturb)) {
  ...
  error = DeeObject_CompareEq(key,item->di_key);
  if unlikely(error < 0) goto err;
  if (!error) continue; /* Non-equal keys. */
  /* Found it! */
  return_true;
 }
 return_false;
...
DeeRoDict_GetItemDef(DeeObject *__restrict self,
 ...
 if (def != ITER_DONE) Dee_Incref(def);
 return def;
err:
 return NULL;
}
...
PRIVATE struct type_seq rodict_seq = {
    ...
    /* .tp_contains  = */(DREF DeeObject *(DCALL *)(DeeObject *__restrict,DeeObject *__restrict))&rodict_contains,
    /* .tp_get       = */(DREF DeeObject *(DCALL *)(DeeObject *__restrict,DeeObject *__restrict))&rodict_getitem,
    /* .tp_del       = */NULL,
    /* .tp_set       = */NULL,
    /* .tp_range_get = */NULL,
    /* .tp_range_del = */NULL,
...
rodict_deepcopy(Dict *__restrict self) {
 DREF Dict *result; size_t i; int temp;
