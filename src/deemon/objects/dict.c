...
DeeDict_NewKeyItemsInherited(size_t num_keyitems, DREF DeeObject **__restrict key_items) {
 ...
 if (!num_keyitems) {
  ...
  if unlikely(!result->d_elem) {
   ...
   if unlikely(!result->d_elem) goto err_r;
  }
  ...
  while (num_keyitems--) {
   ...
   for (;; i = DICT_HASHNX(i,perturb),DICT_HASHPT(perturb)) {
    ...
    if (item->di_key) continue; /* Already in use */
    item->di_hash  = hash;
...
dict_deepload(Dict *__restrict self) {
 ...
 for (i = 0,hash_i = 0; i < item_count; ++hash_i) {
  ...
  if (self->d_elem[hash_i].di_key == NULL) continue;
  if (self->d_elem[hash_i].di_key == dummy) continue;
 ...
 for (i = 0; i < item_count; ++i) {
  ...
  for (;; j = DICT_HASHNX(j,perturb),DICT_HASHPT(perturb)) {
...
PRIVATE void DCALL dict_clear(Dict *__restrict self) {
 struct dict_item *elem; size_t mask;
 DeeDict_LockWrite(self);
 ASSERT((self->d_elem == empty_dict_items) == (self->d_mask == 0));
 ASSERT((self->d_elem == empty_dict_items) == (self->d_size == 0));
 ASSERT(self->d_used <= self->d_size);
...
dict_rehash(Dict *__restrict self, int sizedir) {
 ...
 if (self->d_elem != empty_dict_items) {
  ...
  for (; iter != end; ++iter) {
   ...
   dhash_t i,perturb;
   /* Skip dummy keys. */
   if (!iter->di_key || iter->di_key == dummy) continue;
   perturb = i = iter->di_hash & new_mask;
   for (;; i = DICT_HASHNX(i,perturb),DICT_HASHPT(perturb)) {
    item = &new_vector[i & new_mask];
    if (!item->di_key) break; /* Empty slot found. */
   }
...
again_lock:
 ...
 for (;; i = DICT_HASHNX(i,perturb),DICT_HASHPT(perturb)) {
  ...
  if (item->di_hash != hash) continue; // Non-matching hash
  if (!DeeString_Check(item->di_key)) continue; /* NOTE: This also captures `dummy' */
  if (strcmp(DeeString_STR(item->di_key),key) != 0) continue;
#ifndef CONFIG_NO_THREADS
...
restart:
 ...
 for (;; i = DICT_HASHNX(i,perturb),DICT_HASHPT(perturb)) {
  ...
  if (!item->di_key) break; /* Not found */
  if (item->di_hash != hash) continue; /* Non-matching hash */
  if (item->di_key == dummy) continue; /* Dummy key. */
  item_key   = item->di_key;
  Dee_Incref(item_key);
  DeeDict_LockEndRead(self);
  /* Invoke the compare operator outside of any lock. */
  error = DeeObject_CompareEq(item_key,key);
  Dee_Decref(item_key);
  if (error > 0) return_true; /* Found the item. */
  if unlikely(error < 0)
...
restart:
 ...
 for (;; i = DICT_HASHNX(i,perturb),DICT_HASHPT(perturb)) {
  ...
  error = DeeObject_CompareEq(item_key,key);
  Dee_Decref(item_key);
  if (error > 0) return item_value; /* Found the item. */
...
dict_popitem(Dict *__restrict self,
             DeeObject *__restrict key,
             DeeObject *def) {
 size_t mask; struct dict_item *vector;
 dhash_t i,perturb; int error;
 dhash_t hash = DeeObject_Hash(key);
 DeeDict_LockRead(self);
restart:
...
PRIVATE int DCALL
dict_nsi_updateold(Dict *__restrict self,
                   DeeObject *__restrict key,
                   DeeObject *__restrict value,
...
PRIVATE int DCALL
dict_nsi_insertnew(Dict *__restrict self,
                   DeeObject *__restrict key,
                   DeeObject *__restrict value,
                   DREF DeeObject **poldvalue) {
 int error;
...
dict_repr(Dict *__restrict self) {
 ...
 struct dict_item *iter,*end; bool is_first;
 struct dict_item *vector; size_t mask;
again:
