...
DEFINE_OPERATOR_INVOKE(operator_serialize, &instance_builtin_serialize, &do_inherit_constructor) {
	(void)p_self;
	(void)opname;
	DeeArg_Unpack0(err, argc, argv, OPNAME("serialize"));
	/* TODO: Give user-code control over object serialization
	 *       (probably via something exposed in "rt")
	 *
	 * Alternatively: another idea might be to not give user-code any
	 *                access to any sort of serialization context, and
	 *                just allow user-code to override "operator serialize"
	 *                to return a replacement for the object that is being
	 *                serialized:
	 * >> class MyClass {
	 * >>     this = default;
	 * >>     private member fileOrFilename: File | string;
	 * >>     public operator serialize(): MyClass {
	 * >>         if (fileOrFilename is File) {
	 * >>             local result = copy this;
	 * >>             result.fileOrFilename = fileOrFilename.filename;
	 * >>             return result;
	 * >>         }
	 * >>         return this;
	 * >>     }
	 * >> }
	 * >>
	 * >> local a = MyClass(fileOrFilename: File.open("foo.txt"));
	 * >> local b = deepcopy a; // This would work thanks to the custom "serialize" operator
	 */
	(void)self;
	(void)tp_self;
