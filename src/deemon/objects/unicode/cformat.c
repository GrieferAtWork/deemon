...
DeeString_CFormat(dformatprinter printer,
 ...
 char *iter,*end,*flush_start; DeeObject *in_arg;
...
do_prec_spec:
  ...
  case 'L': goto next_spec; /* Ignored (for now...) */


  {
   unsigned int length,radix,addend; /* integer. */
   union integral intval; size_t printsize;
   char const *dec; bool is_neg;
   char *bufiter,buffer[67]; /* 64-bit binary, w/prefix + sign */
  /* Integer without length specific. */
  case 'b': case 'o': case 'u': case 'd':
  case 'i': case 'x': case 'X': length = 4; --iter; goto do_length_integer;
  case 'p': length = sizeof(void *); --iter; goto do_length_integer;
  /* Various integer length specifiers. */
  case 'h': length = 2; if (*iter == 'h') ++iter,length = 1; goto do_length_integer;
  case 'l': length = 4; if (*iter == 'l') ++iter,length = 8; goto do_length_integer;
  case 'I': length = sizeof(size_t);
            /* */if (*iter == '8') ++iter,length = 1;
            else if (*iter == '1') { if (iter[1] != '6') goto invalid_format; iter += 2,length = 2; }
            else if (*iter == '3') { if (iter[1] != '2') goto invalid_format; iter += 2,length = 4; }
            else if (*iter == '6') { if (iter[1] != '4') goto invalid_format; iter += 2,length = 8; }
            goto do_length_integer;
  case 't': flags |= F_SIGNED; ATTR_FALLTHROUGH
  case 'z': length = sizeof(size_t);
do_length_integer:
   switch (*iter) {
   ...
   case 'i': radix = 10;
             if (ch != 'z')
                 flags |= F_SIGNED;
             break;
   case 'p': if (!(flags&F_HASPREC)) {
              precision = sizeof(void *)*2;
              flags    |= F_HASPREC;
             }
             ATTR_FALLTHROUGH
   case 'X': flags |= F_UPPER; ATTR_FALLTHROUGH
   case 'x': radix = 16; break;
   default: ch = *iter; goto invalid_format;
   }
   ++iter;
   GETARG();
   ...
   do *--bufiter = dec[intval.int_u64 % radix];
   while ((intval.int_u64 /= radix) != 0);
   addend = 0;
   if (flags&F_PREFIX && radix != 10) {
    /* */if (radix == 16) ++addend,*--bufiter = dec[16]; /* X/x */
    else if (radix == 2)  ++addend,*--bufiter = dec[11]; /* B/b */
    ++addend,*--bufiter = '0';
   }
   /* */if (is_neg) ++addend,*--bufiter = '-';
   else if (flags&F_SIGN) ++addend,*--bufiter = '+';
  ...
  case 'q':
   GETARG();
   /* */if (DeeNone_Check(in_arg)) in_arg = (DeeObject *)&str_lpnullrp;
   else if (DeeObject_AssertTypeExact(in_arg,&DeeString_Type)) goto err_m1;
   ...
   if (flags&F_HASPREC) {
    if (str_length > precision)
     ...
     DeeError_Throwf(&DeeError_ValueError,
                     ...
                     "for fixed buffer length %Iu",precision);
     goto err_m1;
    }
   }
