...
err_eof_in_repeat:
   ...
   if (ch32 == '}') {
    ...
    if (piter == pend) goto err_eof_in_repeat;
    ch32 = utf8_readchar((char const **)&piter,pend);
    if (ch32 == '}') {
     result->mc_max = (size_t)-1;
    } else {
...
PRIVATE char *DCALL find_rparen(char *piter, char *pend) {
 ...
 while (piter < pend) {
  char ch = *piter++;
  /* */if (ch == '\\' && piter < pend) ++piter;
  else if (ch == '(') ++paren_recursion;
  else if (ch == ')') {
...
PRIVATE char *DCALL find_pipe(char *piter, char *pend) {
 ...
 while (piter < pend) {
  char ch = *piter++;
  /* */if (ch == '\\' && piter < pend) ++piter;
  else if (ch == '|') { if (!paren_recursion) return piter; }
  else if (ch == '(') ++paren_recursion;
  else if (ch == ')') --paren_recursion;
  else if (ch == '[') {
   while (piter < pend) {
 ...
 for (;;) {
  error = regex_match_impl(pditer,dend,
                          &content_start,content_end,
                           data,context);
  if (error != 0) break; /* Error, or success. */
...
parse_digit_range(char *piter, char *pend,
 ...
 while (piter < pend) {
  if (!has_high && *piter == '<') {
   ...
   if (*piter == '=') {
    ++piter;
    if (parse_u8(&piter,pend,phigh)) goto err;
   } else {
    if (parse_u8(&piter,pend,phigh)) goto err;
    if unlikely(!*phigh) {
     DeeError_Throwf(&DeeError_ValueError,
  ...
  if (!has_low && *piter == '>') {
   ...
   if (*piter == '=') {
    ++piter;
    if (parse_u8(&piter,pend,plow)) goto err;
   } else {
    if (parse_u8(&piter,pend,plow)) goto err;
...
#define CHECK_NONGREEDY_SUFFIX() \
  if (!match.mc_greedy && match_count >= match.mc_min) { \
   ...
   if (error != 0) goto forward_error; /* Suffix match found! */ \
  } \
...
#define DO_CHARACTERWISE_MATCH(expr_using_data_ch) \
  while (match_count < match.mc_max) { \
   ...
   if (!(expr_using_data_ch)) { diter = prev_diter; break; } \
   ++match_count; \
   ++match_length; \
  }
 ...
 int error; size_t match_count;
 size_t match_length = 0;
next:
 ASSERT(piter <= pend);
 if (piter >= pend) goto done;
...
err_lookahead_must_be_nonzero:
    ...
    while (match_count < match.mc_min) {
     error = regex_match_or(&variant_diter,dend,
                             ...
                             REGEX_CONTEXT_FINPAREN);
     if unlikely(error < 0) goto err;
     if (!error) goto nope; /* Insufficient number of matches */
     ++match_count;
    }
    goto next;
   ...
   case '!':
    ...
    while (match_count < match.mc_min) {
     error = regex_match_or(&variant_diter,dend,
                             ...
                             REGEX_CONTEXT_FINPAREN);
     if unlikely(error < 0) goto err;
     if (!error) goto next; /* Miss-match found before it would have been too late. */
     ++match_count;
    }
   ...
   while (match_count < match.mc_max) {
    ...
    error = regex_match_impl(&post_variant_diter,dend,
                             &variant_piter,content_end,
                              ...
                              REGEX_CONTEXT_FEMPTYOK);
    if unlikely(error < 0) { err_backup_v: Dee_Free(backup_v); goto err; }
    if (!error) {
     ...
     while (alt_variant_piter < content_end) {
      ...
      error = regex_match_impl(&alt_variant_diter,dend,
                               &alt_variant_piter,content_end,
                                 ...
                                 * explicitly by jumping to `has_infinite_submatch'. */
                                REGEX_CONTEXT_FEMPTYOK);
      if unlikely(error < 0) goto err_backup_v;
      if (error) {
     ...
     if unlikely(match_count >= backup_a) {
      ...
      if unlikely(!new_vec) {
       ...
       new_vec = (struct match_backup *)Dee_Realloc(backup_v,new_alloc*
                                                    sizeof(struct match_backup));
       if unlikely(!new_vec) goto err_backup_v;
      }
      backup_a = new_alloc;
      backup_v = new_vec;
     }
...
has_infinite_submatch:
 ...
 case ']':
  DeeError_Throwf(&DeeError_ValueError,
                  ...
                  "`%c' in regular expression",(char)ch);
  goto err;
...
match_unicode_trait:
  ...
  case 'n':
   ...
   while (match_count < match.mc_max) {
    ...
    prev_diter = diter;
    data_ch = utf8_readchar((char const **)&diter,dend);
    if (!DeeUni_IsLF(data_ch)) { diter = prev_diter; break; }
 ...
 (DeeUni_Flags(data_ch)&(UNICODE_FALPHA|UNICODE_FLOWER|UNICODE_FUPPER|UNICODE_FTITLE| \
  ...
  case 'B':
   ...
   if (IS_ALNUM_EXTENDED(data_ch) ^ (ch == 'B')) { /* Allow an upper-case `B' to invert the logic. */
    ...
    if (diter >= dend) goto next; /* End of input data */
    temp_diter = diter;
    ...
    if (diter >= dend) goto nope; /* End of input data */
    temp_diter = diter;
    data_ch = utf8_readchar((char const **)&temp_diter,dend);
    if (IS_ALNUM_EXTENDED(data_ch))
...
DeeRegex_Find(/*utf-8*/char const *__restrict data, size_t datalen,
 ...
 ASSERT(datalen != (size_t)-1);
 ASSERT(patternlen != (size_t)-1);
 if unlikely(!patternlen) goto nope;
 rdat.data_start = (char *)data;
 rdat.flags      = flags;
 ...
 if ((candidate > 0x7f || !is_regex_special((char)candidate)) &&
  ...
  while (diter < dend) {
   uint32_t data_ch; char *temp_piter;
   data_ch = utf8_readchar((char const **)&diter,dend);
   ++data_index;
   if (data_ch != candidate) continue;
   if (piter >= pend) {
    /* Got a match! */
