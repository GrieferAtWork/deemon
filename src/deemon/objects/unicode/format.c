...
Formatter_GetUnaryArg(struct formatter *__restrict self,
 ...
 if (DeeUni_IsSymStrt(ch)) {
  ...
  if (!do_eval) {
   ...
   ch = *index_end;
   if (ch == '0') {
    /**/ if (ch == 'b' || ch == 'B') radix = 2,++index_end;
    else if (ch == 'x' || ch == 'X') radix = 16,++index_end;
    else radix = 8;
   }
   for (;;) {
    ch = *index_end;
    ...
    if unlikely(new_index <= index) goto do_variable_length_index;
...
Formatter_GetValue(struct formatter *__restrict self,
 ...
 for (; fmt_end < self->f_end; ++fmt_end) {
  ...
  else if (*fmt_end == '}' && !--recursion) break;
 }
...
#define OBJECT_VECTOR_INIT   { 0, 0, NULL }
...
#define CONFIG_ALLOW_SPACE_IN_FORMAT_EXPRESSION 1
#endif
...
Formatter_GetOne(struct formatter *__restrict self,
                 char **__restrict pfmt_start,
                 bool do_eval) {
 char *fmt_start; DREF DeeObject *result,*new_result;
 ...
 if (**pfmt_start == '(') {
  ...
  ++fmt_start; /* Skip `(' */
#ifdef CONFIG_ALLOW_SPACE_IN_FORMAT_EXPRESSION
  while (DeeUni_IsSpace(*fmt_start)) ++fmt_start;
#endif /* CONFIG_ALLOW_SPACE_IN_FORMAT_EXPRESSION */
  result = Formatter_GetOne(self,&fmt_start,do_eval);
  if unlikely(!result) goto err;
  if (*fmt_start != ')') {
...
next_suffix:
 ...
 case '[':
  ...
  if (*fmt_start == ':') {
   ...
   if unlikely(!index2) { Dee_Decref(index); goto err_r; }
#ifdef CONFIG_ALLOW_SPACE_IN_FORMAT_EXPRESSION
   while (DeeUni_IsSpace(*fmt_start)) ++fmt_start;
#endif /* CONFIG_ALLOW_SPACE_IN_FORMAT_EXPRESSION */
...
#ifdef CONFIG_ALLOW_SPACE_IN_FORMAT_EXPRESSION
    while (DeeUni_IsSpace(*after_dots)) ++after_dots;
#endif /* CONFIG_ALLOW_SPACE_IN_FORMAT_EXPRESSION */
    if (after_dots[0] == ',') {
     int error;
...
parse_second_argument:
  ...
  for (;;) {
   arg = Formatter_GetExpr(self,&fmt_start,do_eval);
   if unlikely(!arg) goto err_call_argv;
#ifdef CONFIG_ALLOW_SPACE_IN_FORMAT_EXPRESSION
   while (DeeUni_IsSpace(*fmt_start)) ++fmt_start;
#endif /* CONFIG_ALLOW_SPACE_IN_FORMAT_EXPRESSION */
   if (fmt_start[0] == '.' &&
    ...
    Dee_Decref(arg);
    if unlikely(extend_error) goto err_call_argv;
    fmt_start += 3;
#ifdef CONFIG_ALLOW_SPACE_IN_FORMAT_EXPRESSION
    while (DeeUni_IsSpace(*fmt_start)) ++fmt_start;
#endif /* CONFIG_ALLOW_SPACE_IN_FORMAT_EXPRESSION */
 ...
 case '?':
  ...
  if unlikely(is_true < 0) goto err_r;
  ++fmt_start; /* Skip `?' */
#ifdef CONFIG_ALLOW_SPACE_IN_FORMAT_EXPRESSION
  while (DeeUni_IsSpace(*fmt_start)) ++fmt_start;
#endif /* CONFIG_ALLOW_SPACE_IN_FORMAT_EXPRESSION */
  ...
  if (*fmt_start == ':') {
   ...
   if (!*fmt_start || *fmt_start == '}') {
    ...
    if unlikely(!ff) { Dee_Decref(tt); goto err_r; }
   }
  } else {
   /* No ff-branch given. */
   ff = Dee_None;
...
#define print(p,s) \
  do{ if unlikely((*printer)(arg,p,s) < 0) goto err; }__WHILE0
 ASSERT(!*self->f_end);
...
check_print_error:
  ...
  if (format_end >= self->f_end) break; /* The format string ends here. */
...
#define print_format(p,s) \
  do{ if unlikely((temp = (*format_printer)(arg,p,s)) < 0) goto err; result += temp; }__WHILE0
 while (self->f_iter < self->f_end) {
  char *format_start;
  char *format_end;
  char ch = *self->f_iter++;
  ...
  if (ch != '{') continue;
  if (self->f_iter == self->f_end) break;
  /* Flush everything up to this point. */
  print_format(self->f_flush_start,
              (size_t)((self->f_iter-1)-self->f_flush_start));
  if (*self->f_iter == '{') {
