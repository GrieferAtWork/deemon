...
next_spec:
  ...
  case 'x': case 'p': radix = 16; goto do_integer_scan;
...
do_integer_scan:
   ...
   if (ignore_data) goto next_format;
  ...
  case 'I':
   if (format >= format_end)
       goto out_formatend;
   /**/ if (format[0] == '8') ++format;
   else if (format[0] == '1' && format < format_end-1 && format[1] == '6') format += 2;
   else if (format[0] == '3' && format < format_end-1 && format[1] == '2') format += 2;
   else if (format[0] == '6' && format < format_end-1 && format[1] == '4') format += 2;
   ATTR_FALLTHROUGH
  case 'h': case 'l': case 'j':
  ...
  /* XXX: What about the %n option?
   *   -> It should be OK if we don't implement that one... */
 ...
 case ' ':
  ...
  if (is_bytes) {
   ...
   while (data < data_end) {
    char *prev_data = data;
    uint32_t data_ch = utf8_readchar((char const **)&data,data_end);
    if (!DeeUni_IsSpace(data_ch)) { data = prev_data; break; }
   }
  }
  goto next_format;
 ...
 case '\n':
  ...
  if (*data == '\r') {
   ...
   if (is_bytes < 0)
       is_bytes = DeeBytes_Check(self->si_scanner->ss_data);
   if (is_bytes) goto out_missmatch;
   /*prev_data = data;*/
   data_ch = utf8_readchar((char const **)&data,data_end);
   if (!DeeUni_IsLF(data_ch)) { /*data = prev_data;*/ goto out_missmatch; }
  }
  goto next_format;
...
ss_iter(StringScanner *__restrict self) {
 ...
 if (DeeString_Check(self->ss_data)) {
  ...
  if unlikely(!result->si_datiter) goto err_r;
 ...
 if (DeeString_Check(self->ss_format)) {
  ...
  if unlikely(!result->si_fmtiter) goto err_r;
