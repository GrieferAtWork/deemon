...
BlackListVarkwds_GetItem(BlackListVarkwds *__restrict self,
	...
	goto missing;
	if
		unlikely(BlackListVarkwds_IsBlackListed(self, name))
	goto missing;
	rwlock_read(&self->vk_lock);
	if
		unlikely(!self->vk_argv)
	{
...
BlackListVarkwds_GetItemString(BlackListVarkwds *__restrict self,
	...
	goto missing;
	if
		unlikely(BlackListVarkwds_IsBlackListedString(self, name, hash))
	goto missing;
	rwlock_read(&self->vk_lock);
	if
		unlikely(!self->vk_argv)
...
BlackListVarkwds_GetItemStringLen(BlackListVarkwds *__restrict self,
	...
	goto missing;
	if
		unlikely(BlackListVarkwds_IsBlackListedStringLen(self, name, namesize, hash))
	goto missing;
	rwlock_read(&self->vk_lock);
	if
		unlikely(!self->vk_argv)
...
again:
	...
	if (!ITER_ISOK(result))
		goto done;
	if
		unlikely(DeeObject_Unpack(result, 2, pair))
	goto err_r;
	Dee_Decref(pair[1]);
