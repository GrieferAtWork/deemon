...
blvi_nsi_nextkey(BlackListVarkwdsIterator *__restrict self) {
 struct kwds_entry *ent;
 ent = blvi_nextiter(self);
 if (!ent) return (DREF DeeStringObject *)ITER_DONE;
...
kwds_FindIndex(DeeKwdsObject *__restrict self,
 ...
 for (;; DeeKwds_MAPNEXT(i,perturb)) {
  ...
  if (DeeString_SIZE(entry->ke_name) != DeeString_SIZE(name)) continue;
...
again:
 ...
 for (;; BlackListVarkwds_BLCKNEXT(i,perturb)) {
  ...
  if (DeeString_SIZE((DeeObject *)str) != DeeString_SIZE(name)) continue;
...
again:
 ...
 for (;; BlackListVarkwds_BLCKNEXT(i,perturb)) {
  ...
  if (DeeString_HASH((DeeObject *)str) != hash)
      continue; /* Strings loaded within the hash-set have always been pre-hashed! */
  if (strcmp(DeeString_STR(str),name) != 0) continue;
  return true; /* It is! */
 }
 /* Check if more arguments need to be loaded. */
 if (ATOMIC_READ(self->vk_load) < self->vk_ckwc) {
...
again:
 ...
 for (;; BlackListVarkwds_BLCKNEXT(i,perturb)) {
  ...
  if (DeeString_HASH((DeeObject *)str) != hash)
      continue; /* Strings loaded within the hash-set have always been pre-hashed! */
  if (DeeString_SIZE((DeeObject *)str) != namesize) continue;
  if (memcmp(DeeString_STR(str),name,namesize * sizeof(char)) != 0) continue;
  return true; /* It is! */
 }
 /* Check if more arguments need to be loaded. */
 if (ATOMIC_READ(self->vk_load) < self->vk_ckwc) {
...
BlackListVarkwds_GetItem(BlackListVarkwds *__restrict self,
 ...
 if unlikely(index == (size_t)-1) goto missing;
 if unlikely(BlackListVarkwds_IsBlackListed(self,name)) goto missing;
...
BlackListVarkwds_GetItemString(BlackListVarkwds *__restrict self,
 ...
 if unlikely(index == (size_t)-1) goto missing;
 if unlikely(BlackListVarkwds_IsBlackListedString(self,name,hash)) goto missing;
 rwlock_read(&self->vk_lock);
 if unlikely(!self->vk_argv) {
  rwlock_endread(&self->vk_lock);
  goto missing;
...
BlackListVarkwds_GetItemStringLen(BlackListVarkwds *__restrict self,
 ...
 if unlikely(index == (size_t)-1) goto missing;
 if unlikely(BlackListVarkwds_IsBlackListedStringLen(self,name,namesize,hash)) goto missing;
 rwlock_read(&self->vk_lock);
 if unlikely(!self->vk_argv) {
  rwlock_endread(&self->vk_lock);
  goto missing;
...
blmi_copy(BlackListMappingIterator *__restrict self,
 ...
 if unlikely(!self->mi_iter) goto err;
...
again:
 ...
 if unlikely(DeeObject_Unpack(result,2,pair)) goto err_r;
 Dee_Decref(pair[1]);
...
again:
 result = DeeObject_IterNext(self->mi_iter);
 if (!ITER_ISOK(result)) goto done_r;
 if unlikely(DeeObject_Unpack(result,2,pair)) goto err_r;
 Dee_Decref(pair[1]);
 if (DeeString_Check(pair[0]) &&
     BlackListMapping_IsBlackListed(self->mi_map,pair[0])) {
...
blm_iter(BlackListMapping *__restrict self) {
 ...
 if unlikely(!result->mi_iter) goto err_r;
 result->mi_map  = self;
 Dee_Incref(self);
