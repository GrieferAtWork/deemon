...
typedef struct {
	PROXY_OBJECT_HEAD2_EX(DeeObject, sui_iter,  /* [1..1][const] The underlying union-set. */
	                      SetUnion,  sui_union) /* [1..1][lock(sui_lock)] The current iterator. */
...
typedef struct {
	PROXY_OBJECT_HEAD2_EX(DeeObject,       sii_iter,       /* [1..1][const] The underlying intersection-set. */
	                      SetIntersection, sii_intersect); /* [1..1][const] An iterator for `sii_intersect->si_a' */
...
typedef struct {
	PROXY_OBJECT_HEAD2(sd_a,  /* [1..1][const] The primary set. */
	                   sd_b); /* [1..1][const] The set of objects removed from `sd_a'. */
} SetDifference;

typedef struct {
	PROXY_OBJECT_HEAD2_EX(DeeObject,     sdi_iter,  /* [1..1][const] The underlying difference-set. */
	                      SetDifference, sdi_diff); /* [1..1][const] An iterator for `sdi_diff->sd_a' */
...
typedef struct {
	PROXY_OBJECT_HEAD2_EX(DeeObject,              ssd_iter,  /* [1..1][const] The underlying set. */
	                      SetSymmetricDifference, ssd_set);  /* [1..1][lock(ssd_lock)] The current iterator. */
#ifndef CONFIG_NO_THREADS
