...
PRIVATE struct type_getset tpconst seq_getsets[] = {
	...
	TYPE_GETTER_AB("each", &DeeSeq_Each,
	               ...
	               "${"
	               /**/ "local locks = { getLock(\"a\"), getLock(\"b\") }; /* Bad example; should use threading.all() */\n"
	               /**/ "with (locks.each) { ... }\n"
	...
	TYPE_GETTER_AB("ids", &SeqIds_Of,
	               ...
	               /**/ "Note that many sequence types construct element objects on-the-fly "
	               /**/ "(including the proxy returned here), in which case successive enumerations "
	               /**/ "of the proxy returned here may yield different IDs, or the same ID may appear "
	               /**/ "multiple times with different effective values during the same enumeration\n"
	...
	TYPE_GETTER("isfrozen", &seq_get_isfrozen,
	            "->?Dbool\n"
	            "Evaluates to true if the effective values of elements of @this Sequence "
	            /**/ "can never change though use of any non-implementation-specific "
	            /**/ "functions/attributes (i.e. anything that doesn't match #C{__*__})"),
...
"}"
"#L{"
/**/ "{\xC2\xB9}The actual return type may be a proxy Sequence that further optimizes the iteration "
