...
INTERN_TPCONST struct type_method tpconst seq_methods[] = {
	...
	TYPE_KWMETHOD(STR_xchitem, &default_seq_xchitem,
	              ...
	              /**/ "default-implementation is provided by ?.:\n"
	              "${"
	              /**/ "function xchitem(index: int, value: Object) {\n"
	              /**/ "	import int, Error from deemon;\n"
	              /**/ "	index = index.operator int();\n"
	              /**/ "	for (local tp: type(this).__mro__) {\n"
	              /**/ "		if (tp === Sequence)\n"
	...
	TYPE_KWMETHOD(STR_remove, &default_seq_remove,
	              ...
	              "For mutable sequences only: Find the first instance of @item and remove it, "
	              /**/ "returning ?t if an element got removed, or ?f if @item could not be found\n"
	              "Depending on the nearest implemented group of operators, "
	              /**/ "one of the following implementations is chosen\n"
	              "For ${operator del[]}:\n"
	              "${"
	              /**/ "function remove(item: int, start: int, end: int, key: Callable): bool {\n"
	              /**/ "	start = start.operator int();\n"
	              /**/ "	end = end.operator int();\n"
	              /**/ "	local mylen = ##this;\n"
	              /**/ "	if (end > mylen)\n"
	              ...
	              "For ${operator del[:]}:\n"
	              "${"
	              /**/ "function remove(item: Object, start: int, end: int, key: Callable): bool {\n"
	              /**/ "	start = start.operator int();\n"
	              /**/ "	end = end.operator int();\n"
	              /**/ "	local mylen = ##this;\n"
	              /**/ "	if (end > mylen)\n"
	...
	TYPE_KWMETHOD(STR_rremove, &default_seq_rremove,
	              ...
	              "#pkey{A key function for transforming Sequence elements}"
	              "#tSequenceError{@this Sequence is immutable}"
	              "For mutable sequences only: Find the last instance of @item and remove it, "
	              /**/ "returning ?t if an element got removed, or ?f if @item could not be found\n"
	              "Depending on the nearest implemented group of operators, "
	              /**/ "one of the following implementations is chosen\n"
	              "For ${operator del[]}:\n"
	              "${"
	              /**/ "function rremove(item: int, start: int, end: int, key: Callable): bool {\n"
	              /**/ "	start = start.operator int();\n"
	              /**/ "	end = end.operator int();\n"
	              /**/ "	local mylen = ##this;\n"
	              /**/ "	if (end > mylen)\n"
	...
	TYPE_KWMETHOD(STR_removeall, &default_seq_removeall,
	              ...
	              "#pkey{A key function for transforming Sequence elements}"
	              "#tSequenceError{@this Sequence is immutable}"
	              "For mutable sequences only: Find all instance of @item and remove "
	              /**/ "them, returning the number of instances found (and consequently removed)\n"
	              "Depending on the nearest implemented group of operators, "
	              /**/ "one of the following implementations is chosen\n"
	              "For ${removeif(should, start, end)}:\n"
	              "${"
	              /**/ "function removeall(item: Object, start: int, end: int, key: Callable): int {\n"
	              /**/ "	start = start.operator int();\n"
	              /**/ "	end = end.operator int();\n"
	              /**/ "	if (start >= end) return 0;\n"
	              /**/ "	if (key is none)\n"
	              /**/ "		return this.removeif(x -\\> item == x, start, end).operator int();\n"
	              /**/ "	item = key(item);\n"
	              /**/ "	return this.removeif(x -\\> item == key(x), start, end).operator int();\n"
	              /**/ "}"
	              "}\n"
	              "For ${rremove(item, start, end, key)}:\n"
	              "${"
	              /**/ "function removeall(item: Object, start: int, end: int, key: Callable): int {\n"
	              /**/ "	start = start.operator int();\n"
	              /**/ "	end = end.operator int();\n"
	              ...
	              /**/ "	while (end > start) {\n"
	              /**/ "		if (key is none) {\n"
	              /**/ "			if (!this.rremove(item, start, end))\n"
	              /**/ "				break;\n"
	              /**/ "		} else {\n"
	              /**/ "			if (!this.rremove(item, start, end, key))\n"
	              /**/ "				break;\n"
	              /**/ "		}\n"
	              /**/ "		++count;\n"
	              /**/ "		--end;\n"
	              ...
	              /**/ "	while (end > start) {\n"
	              /**/ "		if (key is none) {\n"
	              /**/ "			if (!this.remove(item, start, end))\n"
	              /**/ "				break;\n"
	              /**/ "		} else {\n"
	              /**/ "			if (!this.remove(item, start, end, key))\n"
	              /**/ "				break;\n"
	              /**/ "		}\n"
	              /**/ "		++count;\n"
	              /**/ "		--end;\n"
	...
	TYPE_KWMETHOD("blocate", &default_seq_blocate,
	              ...
	              "#pkey{A key function for transforming Sequence elements}"
	              "#tValueError{The Sequence does not contain an itement matching @item}"
	              "Same as ?#bfind, but return the matching itement, rather than its index"),
	TYPE_KWMETHOD("blocateall", &seq_blocateall,
	              "(item,start=!0,end=!-1,key:?DCallable=!N)->?S?O\n"
	              "#pitem{The itement to search for}"
	              "#pkey{A key function for transforming Sequence elements}"
	              "#pstart{The start index for a sub-range to search (clamped by ${##this})}"
	              "#pend{The end index for a sub-range to search (clamped by ${##this})}"
