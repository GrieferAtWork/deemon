...
DeeFile_OpenString(char const *__restrict filename,
 ...
 DREF SystemFile *result; FILE *fp;
 ASSERT(filename);
...
DeeFile_Open(/*String*/DeeObject *__restrict filename, int oflags, int mode) {
 ...
 if unlikely(!utf8_filename) return NULL;
...
#if !defined(_MSC_VER) && \
    !defined(__linux__) && !defined(__KOS__)
   if (!result->sf_handle)
#endif
#endif
   {
...
#define CONFIG_DONT_EXPOSE_FILENO 1
  ...
  * there is no kernel to deal with closed file descriptors,
...
sysfile_isatty(SystemFile *__restrict self,
 ...
 if (DeeArg_Unpack(argc,argv,":isatty"))
     return NULL;
 if (!self->sf_handle) { error_file_closed(self); return NULL; }
