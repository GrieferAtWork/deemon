...
do_realloc:
  entry = (struct late_cmd_option *)Dee_TryRealloc(late_options.lco_optv,new_alloc*
...
find_short_option(struct cmd_option *__restrict options,
 ...
 if unlikely(!name_len) goto done;
 if unlikely(name_len > SHORT_OPTION_MAXLEN) goto done;
 for (; !CMD_OPTION_ISSENTINEL(options); ++options) {
  if ((options->co_flags&req_mask) != req_flags) continue; /* Invalid flags. */
  if (memcmp(options->co_shortnam,name,name_len*sizeof(char))) continue; /* Different name. */
  if (options->co_shortnam[name_len]) continue; /* Name too long. */
  return options;
 }
done:
 return NULL;
...
find_long_option(struct cmd_option *__restrict options,
 ...
 for (; !CMD_OPTION_ISSENTINEL(options); ++options) {
  if ((options->co_flags&req_mask) != req_flags) continue; /* Invalid flags. */
  if (!options->co_longname) continue; /* No long name available. */
  if (memcmp(options->co_longname,name,name_len*sizeof(char))) continue; /* Different name. */
  if (options->co_longname[name_len]) continue; /* Name too long. */
  return options;
 }
done:
 return NULL;
...
cmd_parse(int    *__restrict pargc,
 ...
 for (;;) {
  ...
  cmd = *argv;
#if 0
  if (!*cmd) { --argc; ++argv; continue; } /* Skip empty arguments. */
#endif
  if (!exec_all && cmd[0] != '-')
       break; /* End of command list. */
  if (cmd[0] == '-') ++cmd; /* Skip the initial dash. */
...
check_long:
   ...
   if (eq_sign) {
    ...
    if (opt) { arg = eq_sign+1; goto has_opt; }
   }
   while (cmd_len) {
    --cmd_len;
...
check_short_options:
   ...
   while (cmd_len) {
    ...
    if (opt->co_flags&(CMD_FARG|CMD_FGROUP)) {
     ...
     if (!(opt->co_flags&CMD_FJOINABLE)) continue;
    }
    goto has_opt;
   }
  }
...
has_opt:
  ...
  if (!arg && argc && !(opt->co_flags&CMD_FARGEQ) &&
   ...
   if (opt->co_flags&CMD_FREMAINDER) {
    ...
    size_t argv_len = (size_t)argc; unsigned int i;
  ...
  if (opt->co_flags&CMD_FGROUP) {
   int    sub_argc; char *iter,*argend;
   char **sub_argv,**sub_argv_mirror;
   /* Options sub-group. (comma-separated argument list) */
   if (!arg || !*arg) continue; /* No sub-commands listed. */
   while (*arg == ',') ++arg; /* Skip leading commas. */
