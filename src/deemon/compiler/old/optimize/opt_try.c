 ...
 */
#ifndef GUARD_DEEMON_COMPILER_OLD_OPTIMIZE_OPT_TRY_C
#define GUARD_DEEMON_COMPILER_OLD_OPTIMIZE_OPT_TRY_C 1

#include <deemon/api.h>

#include <deemon/code.h>                  /* Dee_EXCEPTION_HANDLER_FFINALLY */
#include <deemon/compiler/old/ast.h>      /* AST_CONSTEXPR, AST_TRY, CATCH_EXPR_FSECOND, ast, ast_decref, catch_expr */
#include <deemon/compiler/old/optimize.h> /* OPTIMIZE_FASSUME, OPTIMIZE_VERBOSE, ast_*, optimizer_count, optimizer_flags */
...
(DCALL ast_optimize_try)(struct ast_optimize_stack *__restrict stack,
		...
		for (; iter < end; ++iter) {
			if (iter->ce_flags & Dee_EXCEPTION_HANDLER_FFINALLY) {
				...
				if (iter->ce_mask &&
					...
					goto err;
				if (ast_optimize(stack, iter->ce_code, result_used))
					goto err;
			} else {
		...
		for (; iter < end; ++iter) {
			if (iter->ce_mask &&
			    ast_optimize(stack, iter->ce_mask,
			                 !(iter->ce_flags & Dee_EXCEPTION_HANDLER_FFINALLY)))
				goto err;
			if (ast_optimize(stack, iter->ce_code, result_used))
				goto err;
		}
...
#endif /* !GUARD_DEEMON_COMPILER_OLD_OPTIMIZE_OPT_TRY_C */
