 ...
 */
#ifndef GUARD_DEEMON_COMPILER_OLD_LEXER_DOCTEXT_C
#define GUARD_DEEMON_COMPILER_OLD_LEXER_DOCTEXT_C 1
...
do_compile_printer_to_printer(struct Dee_unicode_printer *__restrict result_printer,
	...
	if likely((source_printer->up_flags & Dee_UNICODE_PRINTER_FWIDTH) == STRING_WIDTH_1BYTE) {
		...
		/* Convert to utf-8 */
		rawutf8 = DeeString_AsUtf8(rawtext);
		if unlikely(!rawutf8) {
err_rawtext:
			...
			table_casecorner: {
				 ...
				 *       iter points after that same character! */
				{
					...
					for (;;) {
						 ...
						 * In such a scenario, the position of corner or vertical characters
...
/*do_switch_ch_after_whitespace_or_lf:*/
					...
					if (!second_space && (iter >= end)) {
						/* Shouldn't happen (trailing space/line-feeds are stripped above) */
						goto done_dontflush;
					}
				}
			}
...
#endif /* !GUARD_DEEMON_COMPILER_OLD_LEXER_DOCTEXT_C */
