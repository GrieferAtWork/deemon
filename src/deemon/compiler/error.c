...
#define CONST_STR()  (_temp_string = TPPConst_ToString(ARG(struct TPPConst *)),_temp_string ? _temp_string->s_text : NULL)
#undef FALSE
#ifdef _MSC_VER
#define FALSE         0,0
#else
...
do_realloc_errors:
 new_vector = (DREF DeeCompilerErrorObject **)Dee_TryRealloc(current_parser_errors.pe_errorv,new_alloc*
...
INTERN int DCALL parser_rethrow(bool must_fail) {
 struct except_frame **piter,*iter; uint16_t num_errors;
 ...
 if (current_parser_errors.pe_master) {
  DREF DeeCompilerErrorObject *master; size_t i,count;
...
handle_master:
  ...
  if (master->ce_errorc) {
   ...
   if (master->ce_errorc > 1) {
    ...
    for (; iter != end; ++iter) {
     if (*iter == master) continue; /* Skip the master error. (that one's already been added) */
...
err:
 ...
 if (file->f_prev) {
  ...
  while (next_file->f_kind == TPPFILE_KIND_EXPLICIT &&
         next_file->f_prev) next_file = next_file->f_prev;
...
handle_compiler_warning(struct ast_loc *loc, bool force_fatal,
 ...
 if unlikely(!error->e_message) goto err_error;
