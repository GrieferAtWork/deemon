...
tpp_userstream_fopen(char const *__restrict filename) {
 ...
 /* */if (!result) TPPLexer_SetErr();                   /* Set a lexer error to indicate failure. */
 else if (result == (stream_t)ITER_DONE) result = NULL; /* Return NULL to indicate file-not-found. */
 return result;
}
...
tpp_userstream_fread(DeeFileObject *__restrict self,
 ...
 if (DeeThread_CheckInterrupt()) result = -1;
 else result = DeeFile_Read((DeeObject *)self,buffer,bufsize);
...
again:
 ...
 if (TPPLexer_Current->l_flags & TPPLEXER_FLAG_WANTLF) {
  if (*p == '\r' && p[1] == '\n') p += 2;
  while (result < end && tpp_isspace_nolf(*result)) ++result;
  if (*result != '/') goto set_result;
...
peek_next_keyword(int create_missing) {
 ...
 if unlikely(!tok_begin) return NULL;
...
lowercase_keyword(char const *__restrict name,
 ...
 { char *iter,*end;
   ...
   for (; iter != end; ++iter) *iter = tolower(*iter);
 }
...
INTERN bool DCALL token_replace_lowercase(int create_missing) {
 ...
 if (!TPP_ISKEYWORD(tok)) return false;
 lowername = lowercase_keyword(token.t_kwd->k_name,
                               token.t_kwd->k_size,
                               create_missing);
 if (token.t_kwd == lowername) return false;
...
#define printob(ob) do{if ((temp = DeeObject_Print(ob,printer,arg)) < 0) goto err; result += temp;}__WHILE0
 ...
 file_and_line = TPPLexer_Current->l_flags&TPPLEXER_FLAG_MSVC_MESSAGEFORMAT
               ? LOG_PREFIX "%s(%d,%d) : " : LOG_PREFIX "%s:%d:%d: ";
 ...
 for (i = 0; i < count; ++i) {
  if (self->ce_errorv[i] == self) continue;
  temp = (*printer)(arg,"\n",1);
...
tpp_unknown_file(int mode, char *__restrict filename,
 ...
 DeeStringObject *buffer,*new_buffer; size_t buflen;
 DeeObject *libpath; size_t i; struct TPPKeyword *keyword_entry;
 DREF DeeObject *path,*stream; struct TPPFile *result;
 ...
 for (i = 0; i < DeeList_SIZE(libpath); ++i) {
  size_t req_length; char *dst;
  path = DeeList_GET(libpath,i);
  if (!DeeString_Check(path)) continue; /* Ignore anything that isn't a string. */
  ...
  if unlikely(req_length > buflen) {
   ...
   if unlikely(!new_buffer) goto err_path;
   if (!buffer) {
  ...
  *dst = 0;
  ...
  if (stream != ITER_DONE) { /* Error or success. */
   ...
   if (!keyword_entry) {
    ...
    if unlikely(!keyword_entry) goto err_r;
   }
...
#endif
   if (pkeyword_entry) *pkeyword_entry = keyword_entry;
   return result;
  }
  DeeList_LockRead(libpath);
 }
