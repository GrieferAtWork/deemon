...
gen_guard:
  ...
  if (ast_genasm(self->a_try.t_guard,gflags))
      goto err;
...
#define assert_cleanup  assert_enter
    ...
    /* STACK: a, [b, [c, [d]]] */
    if (message ? (ast_genasm(message,ASM_G_FPUSHRES) || asm_putddi(self))
                :  asm_gpush_none())
        goto err;
    ...
    else if (asm_putddi(self)) goto err;
    /* STACK: a, [b, [c, [d]]] [condition] */
