...
STATIC_ASSERT((ASM16_GETMEMBER_R&0xff) == ASM_GETMEMBER_R);
STATIC_ASSERT((ASM16_DELMEMBER_R&0xff) == ASM_DELMEMBER_R);
STATIC_ASSERT((ASM16_SETMEMBER_R&0xff) == ASM_SETMEMBER_R);
 ...
 if (!(current_assembler.a_flag&ASM_FPEEPHOLE)) {
  ...
  while (iter < end) {
   if (*(iter + 0) == ASM_ADJSTACK) {
    ...
    int16_t offset = ASM_BSWAPSIMM16(*(int16_t *)(iter + 2));
    switch (offset) {
...
INTERN bool DCALL asm_minjmp(void) {
 ...
 for (; iter != end; ++iter) {
  ...
  case R_DMN_DISP16:
   ...
   target  = ASM_BSWAPSIMM16(*(int16_t *)(sc_main.sec_begin + iter->ar_addr));
  ...
  case R_DMN_STCK16:
   ...
   stack_offset = (iter->ar_sym->as_stck -
                   ASM_BSWAPSIMM16(*(int16_t *)(sc_main.sec_begin + iter->ar_addr)));
...
INTERN int DCALL asm_mergestatic(void) {
 ...
 for (; iter != end; ++iter) {
  ...
  static_id = (uint16_t)(ASM_BSWAPIMM16(*imm_addr) + static_offset);
  ...
  ASSERT((uint32_t)static_id == (uint32_t)ASM_BSWAPIMM16(*imm_addr) + (uint32_t)static_offset);
   ...
   *imm_addr = ASM_BSWAPIMM16(static_id);
  }
...
INTERN int DCALL asm_linkstack(void) {
 ...
 for (; iter != end; ++iter) {
  ...
  case R_DMN_STCK16:
   ASSERT(iter->ar_sym->as_stck != ASM_SYM_STCK_INVALID);
   rel_value = iter->ar_sym->as_stck - ASM_BSWAPSIMM16(*(int16_t *)target);
    ...
    *(int16_t *)target = ASM_BSWAPSIMM16((int16_t)rel_value);
   }
   break;
  case R_DMN_STCKAS8:
   ASSERT(iter->ar_sym->as_stck != ASM_SYM_STCK_INVALID);
  ...
  case R_DMN_STCKA16:
   ASSERT(iter->ar_sym->as_stck != ASM_SYM_STCK_INVALID);
   rel_value = ASM_BSWAPIMM16(*(uint16_t *)target) + iter->ar_sym->as_stck;
    ...
    *(uint16_t *)target = ASM_BSWAPIMM16((uint16_t)rel_value);
   }
   break;
...
INTERN int DCALL asm_linktext(void) {
 ...
 for (; iter != end; ++iter) {
  ...
  case R_DMN_ABS16:
   rel_value += iter->ar_addr;
   rel_value += ASM_BSWAPIMM16(*(uint16_t *)target);
  ...
  case R_DMN_ABS32:
   rel_value += iter->ar_addr;
   rel_value += ASM_BSWAPIMM32(*(uint32_t *)target);
   *(uint32_t *)target = ASM_BSWAPIMM32((uint32_t)rel_value);
   break;
  case R_DMN_ABSS8:
   rel_value += iter->ar_addr;
   ATTR_FALLTHROUGH
   ...
   *(int8_t *)target = (int8_t)rel_value;
   break;
  case R_DMN_DISP16:
   rel_value += ASM_BSWAPSIMM16(*(int16_t *)target);
   if unlikely(rel_value < INT16_MIN ||
               rel_value > INT16_MAX)
      goto trunc;
   *(int16_t *)target = ASM_BSWAPSIMM16((int16_t)rel_value);
   break;
  case R_DMN_DISP32:
   rel_value += ASM_BSWAPSIMM32(*(int32_t *)target);
   if unlikely(rel_value < INT32_MIN ||
               rel_value > INT32_MAX)
      goto trunc;
   *(int32_t *)target = ASM_BSWAPSIMM32((int32_t)rel_value);
   break;
  case R_DMN_STCK8:
   ASSERT(iter->ar_sym->as_stck != ASM_SYM_STCK_INVALID);
   rel_value = iter->ar_sym->as_stck - *(int8_t *)target;
...
INTERN int (DCALL asm_putimm8_16)(instruction_t instr, uint8_t imm8_1, uint16_t imm16_2) {
  ...
  *            (result + 0) = instr;
  *(uint8_t  *)(result + 1) = imm8_1;
  *(uint16_t *)(result + 2) = ASM_BSWAPIMM16(imm16_2);
  return 0;
 }
 return -1;
}
INTERN int (DCALL asm_putimm16)(instruction_t instr, uint16_t imm16) {
 ...
 if likely(result) {
  *            (result + 0) = instr;
  *(uint16_t *)(result + 1) = ASM_BSWAPIMM16(imm16);
  return 0;
 }
 return -1;
}
INTERN int (DCALL asm_putimm16_8)(instruction_t instr, uint16_t imm16_1, uint8_t imm8_2) {
 ...
 if likely(result) {
  *            (result + 0) = instr;
  *(uint16_t *)(result + 1) = ASM_BSWAPIMM16(imm16_1);
...
INTERN int (DCALL asm_putimm16_16)(instruction_t instr, uint16_t imm16_1, uint16_t imm16_2) {
  ...
  *            (result + 0) = instr;
  *(uint16_t *)(result + 1) = ASM_BSWAPIMM16(imm16_1);
  *(uint16_t *)(result + 3) = ASM_BSWAPIMM16(imm16_2);
  return 0;
 }
 return -1;
}
INTERN int (DCALL asm_putimm16_8_16)(instruction_t instr, uint16_t imm16_1, uint8_t imm8_2, uint16_t imm16_3) {
  ...
  *(uint16_t *)(result + 1) = ASM_BSWAPIMM16(imm16_1);
  *(uint8_t  *)(result + 3) = imm8_2;
  *(uint16_t *)(result + 4) = ASM_BSWAPIMM16(imm16_3);
  return 0;
 }
 return -1;
}
...
INTERN int (DCALL asm_putimm32)(instruction_t instr, uint32_t imm32) {
 ...
 if likely(result) {
  *            (result + 0) = instr;
  *(uint32_t *)(result + 1) = ASM_BSWAPIMM32(imm32);
  return 0;
 }
 return -1;
}
INTERN int (DCALL asm_putsid16)(uint16_t instr, uint16_t sid) {
  ...
  *(uint16_t *)(result + 2) = ASM_BSWAPIMM16(sid);
...
asm_do_gjmp(instruction_t instr,
 ...
 if unlikely(current_assembler.a_flag&ASM_FBIGCODE) {
   ...
   *           (data + 1) = instr;
   *(int32_t *)(data + 2) = ASM_BSWAPSIMM32(-4); /* -4 to adjust for the ip offset of the immediate value itself. */
   ...
   *(int32_t *)(data + 6) = ASM_BSWAPSIMM32(-4); /* -4 to adjust for the ip offset of the immediate value itself. */
  } else {
   /* >>    jnX   1f     // 2
    * >>    jmp32 target // 6
    * >>1:
   ...
   *(int32_t *)(data + 4) = ASM_BSWAPSIMM32(-4); /* -4 to adjust for the ip offset of the immediate value itself. */
  }
 ...
 *(int16_t *)(data + 1) = ASM_BSWAPSIMM16(-2); /* -2 to adjust for the ip offset of the immediate value itself. */
 ...
 *(uint16_t *)(data + 2) = ASM_BSWAPIMM16(current_assembler.a_stackcur);
 rel->ar_addr = asm_ip()-2;
...
asm_newref(struct symbol *__restrict ref_sym) {
 ...
 if (result == current_assembler.a_refa) {
  ...
  if unlikely(new_size <= result) new_size = UINT16_MAX;
do_realloc:
  new_vector = (struct symbol **)Dee_TryRealloc(current_assembler.a_refv,new_size*
 ...
 current_assembler.a_refv[result] = ref_sym;
 return result;
}
...
asm_gsymid(struct symbol *__restrict sym) {
 ...
 ASSERT(sym);
 ASSERT(sym->sym_class == SYM_CLASS_VAR);
 ...
 for (;; i = MODULE_HASHNX(i,perturb),MODULE_HASHPT(perturb)) {
  ...
  if unlikely(!iter->ss_name) return -1;
  iter->ss_doc          = sym->sym_var.sym_doc;
...
asm_rsymid(struct symbol *__restrict sym) {
 ...
 ASSERT(sym);
 ASSERT(sym->sym_class == SYM_CLASS_REF);
...
asm_grsymid(struct symbol *__restrict sym) {
 ...
 while (i) {
  --i;
  if (current_assembler.a_refv[i] == sym)
      return i;
 }
...
asm_esymid(struct symbol *__restrict sym) {
 ...
 ASSERT(sym);
 ASSERT(sym->sym_class == SYM_CLASS_EXTERN);
 if (sym->sym_flag&SYM_FEXTERN_ALLOC)
     return sym->sym_extern.sym_modid;
 module = sym->sym_extern.sym_module;
 if (sym->sym_extern.sym_modsym->ss_flags & MODSYM_FEXTERN) {
  ASSERT(sym->sym_extern.sym_modsym->ss_extern.ss_impid < module->mo_importc);
  module = module->mo_importv[sym->sym_extern.sym_modsym->ss_extern.ss_impid];
 }
...
asm_msymid(struct symbol *__restrict sym) {
 int32_t result;
 ASSERT(sym);
 ASSERT(sym->sym_class == SYM_CLASS_MODULE);
 if (sym->sym_flag&SYM_FMODULE_ALLOC)
     return sym->sym_module.sym_modid;
 result = asm_newmodule(sym->sym_extern.sym_module);
 if unlikely(result < 0) goto end;
 ASSERT(result <= UINT16_MAX);
 /* Cache the module ID within the symbol. */
...
restart:
 /* Generate code for copying modified arguments into locals. */
 {
  ...
  for (; iter != end; ++iter) {
   while ((sym = *iter)->sym_class == SYM_CLASS_ALIAS)
       *iter = sym->sym_alias.sym_alias;
...
do_savearg:
    ...
    goto do_savearg;
              ...
              sym->sym_arg.sym_index == current_basescope->bs_argc_max &&
...
code_compile(DeeAstObject *__restrict code_ast, uint16_t flags,
             ...
             /*out:inherit*/struct symbol ***__restrict prefv) {
