...
asm_invoke_operand_print(struct asm_invoke_operand *__restrict self,
 ...
 case OPERAND_CLASS_POP: raw_operand_string = DeeString_STR(&str_pop); goto do_raw_string;
 ...
 case OPERAND_CLASS_NONE         : raw_operand_string = DeeString_STR(&str_none); goto do_raw_string;
 case OPERAND_CLASS_FOREACH      : raw_operand_string = "foreach"; goto do_raw_string;
 case OPERAND_CLASS_EXCEPT       : raw_operand_string = DeeString_STR(&str_except); goto do_raw_string;
 ...
 case OPERAND_CLASS_THIS         : raw_operand_string = DeeString_STR(&str_this); goto do_raw_string;
 case OPERAND_CLASS_THIS_MODULE  : raw_operand_string = DeeString_STR(&str_this_module); goto do_raw_string;
...
unknown_encoding:
 ...
 case ASM_OP_NONE:
  /* None-operand */
  if (!AST_ISNONE(self)) goto next_option;
  if (ast_genasm(self,ASM_G_FNORMAL)) goto err;
  result = &str_none;
  Dee_Incref(result);
  break;
...
abs_stack_any:
 ...
 case ASM_OP_CONST:
  ...
  if (!asm_allowconst(self->a_constexpr)) goto next_option;
...
write_regular_local:
 ...
 case ASM_OP_LOCAL_GEN:
  ...
  if (mode != OPTION_MODE_OUTPUT) {
   ...
   if (asm_gmov_sym_ast(&temp,self,self)) goto err;
  }
 ...
 case ASM_OP_TRUE:
  if (self->a_type != AST_CONSTEXPR) goto next_option;
  if (self->a_constexpr != Dee_True) goto next_option;
  result = &str_true;
  Dee_Incref(result);
  break;
 ...
 case ASM_OP_FALSE:
  if (self->a_type != AST_CONSTEXPR) goto next_option;
  if (self->a_constexpr != Dee_False) goto next_option;
  result = &str_false;
  Dee_Incref(result);
  break;
 ...
 case ASM_OP_IMMZERO:
  if (self->a_type != AST_CONSTEXPR) goto next_option;
  if (!DeeInt_Check(self->a_constexpr)) goto next_option;
  if (!DeeInt_TryAsU32(self->a_constexpr,&value)) goto next_option;
...

 {
  ...
  if (self->a_type != AST_CONSTEXPR) goto next_option;
  if (!DeeInt_Check(self->a_constexpr)) goto next_option;
  if (!DeeInt_TryAsS64(self->a_constexpr,&intval)) goto next_option;
...

 {
  ...
  if (self->a_type != AST_CONSTEXPR) goto next_option;
  if (!DeeInt_Check(self->a_constexpr)) goto next_option;
  if (!DeeInt_TryAsU64(self->a_constexpr,&intval)) goto next_option;
...
compile_operator(struct asm_operand *__restrict op, bool is_output) {
 ...
 if (is_output) {
  ...
  if (asm_gpop_expr(expr)) goto err;
 } else {
  if (strcmp(format,"x") != 0)
      goto err_illegal_mode;
  if (ast_genasm(expr,ASM_G_FNORMAL)) goto err;
 }
done:
 return 0;
err:
