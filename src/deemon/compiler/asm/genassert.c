...
#define assert_cleanup  assert_enter
  ...
  else if (asm_putddi(ddi_ast)) goto err;
  /* STACK: a, [b, [c, [d]]] [condition] */
  if ((gflags & ASM_G_FPUSHRES) && asm_grrot(argc+1)) goto err;
...
#undef assert_cleanup
 ...
 if ((gflags & ASM_G_FPUSHRES) && asm_gpop()) goto err; /* The asserted expression. */
 argc = 0;
