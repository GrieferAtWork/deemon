...
get_scope(DeeCompilerScopeObject *scope) {
 if (DeeNone_Check(scope)) return current_scope;
...
ast_makesym(DeeCompilerObject *__restrict self,
 ...
 if (DeeObject_AssertTypeExact((DeeObject *)sym,&DeeCompilerSymbol_Type))
     goto done;
 if unlikely(sym->ci_compiler != self) { err_invalid_symbol_compiler(sym); goto done; }
 if unlikely(!sym->ci_value) { err_compiler_item_deleted((DeeCompilerItemObject *)sym); goto done; }
...
#define IS_FLAG(x) (flag_length == COMPILER_STRLEN(x) && memcmp(flags,x,COMPILER_STRLEN(x)) == 0)
   /**/ if (IS_FLAG("finally")) result->ce_flags |= EXCEPTION_HANDLER_FFINALLY;
   else if (IS_FLAG("interrupt")) result->ce_flags |= EXCEPTION_HANDLER_FINTERPT;
   else {
...
unpack_catch_expression(DeeObject *__restrict triple,
 ...
 /* (:string flags, #ast mask, #ast code) */
 DeeCompilerAstObject *args[3];
 if (DeeObject_Unpack(triple,3,(DeeObject **)args))
     goto err;
 if (!DeeNone_Check(args[1])) {
  if (DeeObject_AssertTypeExact((DeeObject *)args[1],&DeeCompilerAst_Type)) goto err;
  if unlikely(args[1]->ci_compiler != DeeCompiler_Current) { err_invalid_ast_compiler(args[1]); goto err; }
  if unlikely(args[1]->ci_value->a_scope->s_base != base_scope) { err_invalid_ast_basescope(args[1],base_scope); goto err; }
 }
 if (DeeObject_AssertTypeExact((DeeObject *)args[2],&DeeCompilerAst_Type)) goto err;
 if unlikely(args[2]->ci_compiler != DeeCompiler_Current) { err_invalid_ast_compiler(args[2]); goto err; }
 if unlikely(args[2]->ci_value->a_scope->s_base != base_scope) { err_invalid_ast_basescope(args[2],base_scope); goto err; }
 /* Parse flags. */
...
unpack_catch_expressions(DeeObject *__restrict handlers,
 ...
 size_t catch_c,catch_a,i; int error;
 ...
 if (catch_c != DEE_FASTSEQ_NOTFAST) {
  ...
  if unlikely(!catch_v) goto done;
  for (i = 0; i < catch_c; ++i) {
   elem = DeeFastSeq_GetItem(handlers,i);
   if unlikely(!elem) goto err_fast;
   error = unpack_catch_expression(elem,&catch_v[i],base_scope);
   Dee_Decref(elem);
   if unlikely(error) goto err_fast;
  }
  goto done;
err_fast:
  while (i--) {
  ...
  if (catch_c >= catch_a) {
   ...
   if unlikely(!new_catch_v) {
    ...
    new_catch_v = (struct catch_expr *)Dee_Realloc(catch_v,new_catch_a*
                                                   sizeof(struct catch_expr));
    if unlikely(!new_catch_v) goto err_catch_elem;
   }
   catch_v = new_catch_v;
   catch_a = new_catch_a;
  }
 ...
 Dee_Decref(iterator);
 if unlikely(!elem) goto err_catch;
 /* Release unused memory. */
 if (catch_c != catch_a) {
  struct catch_expr *new_catch_v;
  new_catch_v = (struct catch_expr *)Dee_TryRealloc(catch_v,catch_c*
                                                    sizeof(struct catch_expr));
  if likely(new_catch_v) catch_v = new_catch_v;
 }
done:
 *pcatch_c = catch_c;
 return catch_v;
...
parse_loop_flags(char const *__restrict flags,
 ...
 while (*flags) {
  ...
  if (flag_length) {
#define IS_FLAG(x) (flag_length == COMPILER_STRLEN(x) && memcmp(flags,x,COMPILER_STRLEN(x)) == 0)
   /**/ if (IS_FLAG("foreach"))  *presult |= AST_FLOOP_FOREACH;
   else if (IS_FLAG("postcond")) *presult |= AST_FLOOP_POSTCOND;
   else if (IS_FLAG("unlikely")) *presult |= AST_FLOOP_UNLIKELY;
   else {
    return DeeError_Throwf(&DeeError_ValueError,
                           "Unknown loop flag %$q",
                           flag_length,
...
ast_makeloop(DeeCompilerObject *__restrict self,
 ...
 /* "(string flags,ast elem=none,ast iter,ast loop=none,scope scope=none)->ast\n"
...
check_next:
  ...
  if unlikely(DeeObject_AssertTypeExact((DeeObject *)next,&DeeCompilerAst_Type)) goto done;
  ...
  if unlikely((ast_scope = get_scope(scope)) == NULL)
     goto done;
  if (!DeeNone_Check(next)) goto check_next;
 }
 if (!DeeNone_Check(cond)) {
  if unlikely(DeeObject_AssertTypeExact((DeeObject *)cond,&DeeCompilerAst_Type)) goto done;
  if unlikely(cond->ci_compiler != DeeCompiler_Current) { err_invalid_ast_compiler(cond); goto done; }
  if unlikely(cond->ci_value->a_scope->s_base != ast_scope->s_base) { err_invalid_ast_basescope(cond,ast_scope->s_base); goto done; }
 }
 if (!DeeNone_Check(loop)) {
  if unlikely(DeeObject_AssertTypeExact((DeeObject *)loop,&DeeCompilerAst_Type)) goto done;
...
ast_makeconditional(DeeCompilerObject *__restrict self,
 ...
 if unlikely(DeeObject_AssertTypeExact((DeeObject *)cond,&DeeCompilerAst_Type)) goto done;
 if unlikely(cond->ci_compiler != self) { err_invalid_ast_compiler(cond); goto done; }
 if unlikely(cond->ci_value->a_scope->s_base != ast_scope->s_base) { err_invalid_ast_basescope(cond,ast_scope->s_base); goto done; }
 if (!DeeNone_Check(tt)) {
  if unlikely(DeeObject_AssertTypeExact((DeeObject *)tt,&DeeCompilerAst_Type)) goto done;
  if unlikely(tt->ci_compiler != self) { err_invalid_ast_compiler(tt); goto done; }
  if unlikely(tt->ci_value->a_scope->s_base != ast_scope->s_base) { err_invalid_ast_basescope(tt,ast_scope->s_base); goto done; }
 ...
 if (!DeeNone_Check(ff)) {
  if unlikely(DeeObject_AssertTypeExact((DeeObject *)ff,&DeeCompilerAst_Type)) goto done;
...
get_operator_id(DeeObject *__restrict opid, uint16_t *__restrict presult) {
  ...
  *presult = Dee_OperatorFromName(NULL,name);
  if (*presult != (uint16_t)-1) return 0;
  /* Resolve special operator names. */
  switch (name[0]) {
  case '+':
   if (name[1]) goto unknown_str;
  ...
  case '[':
   if (name[1] == ':') {
    if (name[2] != ']') goto unknown_str;
    if (name[3]) goto unknown_str;
...
ast_makeoperatorfunc(DeeCompilerObject *__restrict self,
                     size_t argc, DeeObject **__restrict argv,
                     DeeObject *kw) {
 /* "(string name,ast binding=none,scope scope=none)->ast\n"
 ...
 if (!DeeNone_Check(binding)) {
  if unlikely(DeeObject_AssertTypeExact((DeeObject *)binding,&DeeCompilerAst_Type)) goto done;
...
parse_operator_flags(char const *__restrict flags,
 ...
 while (*flags) {
  ...
  if (flag_length) {
#define IS_FLAG(x) (flag_length == COMPILER_STRLEN(x) && memcmp(flags,x,COMPILER_STRLEN(x)) == 0)
   /**/ if (IS_FLAG("post"))         *presult |= AST_OPERATOR_FPOSTOP;
   else if (IS_FLAG("varargs"))      *presult |= AST_OPERATOR_FVARARGS;
   else if (IS_FLAG("maybeprefix"))  *presult |= AST_OPERATOR_FMAYBEPFX;
   else if (IS_FLAG("dontoptimize")) *presult |= AST_OPERATOR_FDONTOPT;
   else {
    return DeeError_Throwf(&DeeError_ValueError,
...
ast_makeoperator(DeeCompilerObject *__restrict self,
                 size_t argc, DeeObject **__restrict argv,
                 DeeObject *kw) {
 /* "(string name,ast a,ast b=none,ast c=none,ast d=none,string flags=\"\",scope=none)->ast\n"
 ...
 if unlikely(DeeObject_AssertTypeExact((DeeObject *)a,&DeeCompilerAst_Type)) goto done;
 if unlikely(a->ci_compiler != self) { err_invalid_ast_compiler(a); goto done; }
 if unlikely(a->ci_value->a_scope->s_base != ast_scope->s_base) { err_invalid_ast_basescope(a,ast_scope->s_base); goto done; }
 if (!DeeNone_Check(b)) {
  if unlikely(DeeObject_AssertTypeExact((DeeObject *)b,&DeeCompilerAst_Type)) goto done;
  if unlikely(b->ci_compiler != self) { err_invalid_ast_compiler(b); goto done; }
  if unlikely(b->ci_value->a_scope->s_base != ast_scope->s_base) { err_invalid_ast_basescope(b,ast_scope->s_base); goto done; }
  if (!DeeNone_Check(c)) {
   if unlikely(DeeObject_AssertTypeExact((DeeObject *)c,&DeeCompilerAst_Type)) goto done;
   if unlikely(c->ci_compiler != self) { err_invalid_ast_compiler(c); goto done; }
   if unlikely(c->ci_value->a_scope->s_base != ast_scope->s_base) { err_invalid_ast_basescope(c,ast_scope->s_base); goto done; }
   if (!DeeNone_Check(d)) {
    if unlikely(DeeObject_AssertTypeExact((DeeObject *)d,&DeeCompilerAst_Type)) goto done;
    if unlikely(d->ci_compiler != self) { err_invalid_ast_compiler(d); goto done; }
    if unlikely(d->ci_value->a_scope->s_base != ast_scope->s_base) { err_invalid_ast_basescope(d,ast_scope->s_base); goto done; }
   }
  }
 }
 switch (id) {
...
ast_makeaction(DeeCompilerObject *__restrict self,
               size_t argc, DeeObject **__restrict argv,
               DeeObject *kw) {
 /* "(string name,ast a=none,ast b=none,ast c=none,bool mustrun=true,scope=none)->ast\n" */
 ...
 if (!mustrun) result_ast->a_flag |= AST_FACTION_MAYBERUN;
...
PRIVATE DREF DeeObject *DCALL ast_makeclass(DeeCompilerObject *__restrict self, size_t argc, DeeObject **__restrict argv, DeeObject *kw) { (void)self; (void)argc; (void)argv; (void)kw; DERROR_NOTIMPLEMENTED(); return NULL; }
PRIVATE DREF DeeObject *DCALL ast_makelabel(DeeCompilerObject *__restrict self, size_t argc, DeeObject **__restrict argv, DeeObject *kw) { (void)self; (void)argc; (void)argv; (void)kw; DERROR_NOTIMPLEMENTED(); return NULL; }
PRIVATE DREF DeeObject *DCALL ast_makegoto(DeeCompilerObject *__restrict self, size_t argc, DeeObject **__restrict argv, DeeObject *kw) { (void)self; (void)argc; (void)argv; (void)kw; DERROR_NOTIMPLEMENTED(); return NULL; }
PRIVATE DREF DeeObject *DCALL ast_makeswitch(DeeCompilerObject *__restrict self, size_t argc, DeeObject **__restrict argv, DeeObject *kw) { (void)self; (void)argc; (void)argv; (void)kw; DERROR_NOTIMPLEMENTED(); return NULL; }
PRIVATE DREF DeeObject *DCALL ast_makeassembly(DeeCompilerObject *__restrict self, size_t argc, DeeObject **__restrict argv, DeeObject *kw) { (void)self; (void)argc; (void)argv; (void)kw; DERROR_NOTIMPLEMENTED(); return NULL; }
...
INTERN struct type_method compiler_methods[] = {
    ...
    { "maketry", (DREF DeeObject *(DCALL *)(DeeObject *__restrict,size_t,DeeObject **__restrict))&ast_maketry,
      DOC("(ast guard,{(string,ast,ast)...} handlers,scope scope=none)->ast\n"
          ...
          "The flags in this triple is a $\",\"-seperated string containing "
    ...
    { "makeloop", (DREF DeeObject *(DCALL *)(DeeObject *__restrict,size_t,DeeObject **__restrict))&ast_makeloop,
      DOC("(string flags,ast elem=none,ast iter,ast loop=none,scope scope=none)->ast\n"
          ...
          "$\",\"-seperated string containing zero or more of the following options, "
    ...
    { "makeconditional", (DREF DeeObject *(DCALL *)(DeeObject *__restrict,size_t,DeeObject **__restrict))&ast_makeconditional,
      DOC("(ast cond,ast tt=none,ast ff=none,string flags=\"\",scope scope=none)->ast\n"
          ...
          "The given @flags is a $\",\"-seperated string containing zero or "
    ...
    { "makeaction", (DREF DeeObject *(DCALL *)(DeeObject *__restrict,size_t,DeeObject **__restrict))&ast_makeaction,
      DOC("(string name,ast a=none,ast b=none,ast c=none,bool mustrun=true,scope=none)->ast\n"
          ...
          "$\"print\"|${print a...,;}|1|Print the individual elements of a sequence @a, seperated by spaces\n"
          "$\"println\"|${print a...;}|1|Print the individual elements of a sequence @a, seperated by spaces, and followed by a line-feed\n"
