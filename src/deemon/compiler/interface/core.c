...
again:
 ...
 if (self->cp_items.ci_list) {
  ...
  for (; result; result = result->ci_next) {
   if (result->ci_value != value) continue;
   if unlikely(!Dee_IncrefIfNotZero(result)) continue;
 ...
 if (self->cp_items.ci_list) {
  ...
  for (; new_result; new_result = new_result->ci_next) {
   if likely(new_result->ci_value != value) continue;
   if unlikely(!Dee_IncrefIfNotZero(new_result)) continue;
 ...
 if (self->cp_items.ci_size >= self->cp_items.ci_mask) {
  ...
  /* Re-hash the old map. */
  if (self->cp_items.ci_list) {
   DeeCompilerItemObject *iter,*next; size_t i;
