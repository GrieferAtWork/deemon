...
dec_allocstr(void const *__restrict data, size_t n_bytes) {
 ...
 result = (uint8_t *)memmem(sec->ds_begin,(size_t)(sec->ds_iter-sec->ds_begin),
                            data,n_bytes);
 if (!result) {
...
dec_reuseglobal_define(struct dec_sym *__restrict sym, size_t n_bytes) {
 ...
 if (current_dec.dw_flags&DEC_WRITE_FREUSE_GLOBAL) {
  ...
  DEC_FOREACH_SECTION(sec) {
   ...
   if (existing_data == data) continue; /* Skip the one we're trying to delete. */
...
do_realloc:
  ...
  if unlikely(!result) {
   if (new_alloc != sec->ds_relc+1) { new_alloc = sec->ds_relc+1; goto do_realloc; }
   if (Dee_CollectMemory(new_alloc*sizeof(struct dec_rel))) goto do_realloc;
   goto done;
  }
  sec->ds_relv = result;
...
#define PRINTF(...) do if (DeeFile_Printf(DeeFile_DefaultStddbg,__VA_ARGS__) < 0) goto err; __WHILE0
 unsigned int root_id; size_t total_size = 0;
...
INTERN int (DCALL dec_createfp)(DeeModuleObject *__restrict module,
 ...
 struct dec_writer old_dec; int result;
...
INTERN int (DCALL dec_create)(DeeModuleObject *__restrict module) {
 int result; struct dec_writer old_dec;
 ...
 if ((dec_filename = config_filename) == NULL) {
  ...
  for (;;) {
   if (!dec_filelen) goto cannot_create;
   if (dec_filestr[dec_filelen-1] == '.') break;
   if (dec_filestr[dec_filelen-1] == SEP) goto cannot_create;
   --dec_filelen;
  }
  dec_filename = DeeString_NewBuffer(dec_filelen+4);
  if unlikely(!dec_filename) goto err;
  {
   ...
   else dec_filestart = dec_filestr;
 ...
 if (DeeString_Check(dec_filename)) {
  ...
  output_fp = DeeFile_Open(dec_filename,
                           ...
                           0644);
  ASSERT_OBJECT_TYPE_EXACT(dec_filename,&DeeString_Type);
  if unlikely(!output_fp) goto err_filename;
  /* Write all DEC data to the provided file stream. */
  ASSERT_OBJECT_TYPE_EXACT(dec_filename,&DeeString_Type);
  ...
  if (result) {
   ...
   if likely(wname) DeleteFileW(wname);
   else {
