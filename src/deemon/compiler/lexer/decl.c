...
decl_ast_parse_unary_head(struct decl_ast *__restrict self) {
 ...
 case KWD_type:
  ...
  if unlikely(!type_expr) goto err;
...
err_type_expr:
 ...
 case '(':
   ...
   if (elemc >= elema) {
    ...
    if unlikely(!new_elemv) {
     ...
     new_elemv = (struct decl_ast *)Dee_Realloc(elemv,elema *
                                                sizeof(struct decl_ast));
     if unlikely(!new_elemv) goto err_elemv;
    }
    elemv = new_elemv;
   }
  }
  ...
  TPPLexer_Current->l_flags |= old_flags & TPPLEXER_FLAG_WANTLF;
  if unlikely(likely(tok == ')') ? (yield() < 0) :
              WARN(W_EXPECTED_RPAREN_AFTER_TUPLE)) {
   old_flags = 0;
   goto err_elemv;
  }
...
err_elemv:
 ...
 case '{':
  ...
  if unlikely(!decl_seq) goto err_flags;
...
err_seq:
 ...
 case KWD___nth:
  ...
  if unlikely(!nth_expr) goto err_flags;
  TPPLexer_Current->l_flags |= old_flags & TPPLEXER_FLAG_WANTLF;
...
err_nth:
  ...
  if (TPP_ISKEYWORD(tok)) {
   ...
   if (nth_expr->a_type == AST_CONSTEXPR &&
    ...
    if (WARN(W_EXPECTED_CONSTANT_AFTER_NTH)) goto err_nth;
   }
   ast_decref(nth_expr);
 ...
 case '.':
  ...
  if (self->da_type == DAST_CONST) {
   ...
   modsym = DeeModule_GetSymbolString(self->da_symbol->s_module,token.t_kwd->k_name,
                                      hash_ptr(token.t_kwd->k_name,token.t_kwd->k_size));
   if likely(modsym) {
    struct symbol *new_symbol;
    new_symbol = new_unnamed_symbol_in_scope(self->da_symbol->s_scope);
    if unlikely(!new_symbol) goto err_r;
