...
convert_operator_name(uint16_t *__restrict pname, size_t argc) {
 ...
 if (*pname >= AST_OPERATOR_MIN &&
  ...
  case AST_OPERATOR_POS_OR_ADD:
   if (argc == 1) { *pname = OPERATOR_POS; goto ok; }
   if (argc == 2) { *pname = OPERATOR_ADD; goto ok; }
   break;
  case AST_OPERATOR_NEG_OR_SUB:
   if (argc == 1) { *pname = OPERATOR_NEG; goto ok; }
   if (argc == 2) { *pname = OPERATOR_SUB; goto ok; }
   break;
  case AST_OPERATOR_GETITEM_OR_SETITEM:
   if (argc == 2) { *pname = OPERATOR_GETITEM; goto ok; }
   if (argc == 3) { *pname = OPERATOR_SETITEM; goto ok; }
   break;
  case AST_OPERATOR_GETRANGE_OR_SETRANGE:
   if (argc == 3) { *pname = OPERATOR_GETRANGE; goto ok; }
   if (argc == 4) { *pname = OPERATOR_SETRANGE; goto ok; }
   break;
  case AST_OPERATOR_GETATTR_OR_SETATTR:
   if (argc == 2) { *pname = OPERATOR_GETATTR; goto ok; }
   if (argc == 3) { *pname = OPERATOR_SETATTR; goto ok; }
   break;
  default: break;
  }
  return false;
...
ast_build_operator(uint16_t name, uint16_t flags,
 ...
 if (args->ast_type == AST_CONSTEXPR &&
  ...
  if likely(argc < 4 && argc != 0) {
   ...
   if (argc >= 1 && (argv[0] = ast_constexpr(DeeTuple_GET(tuple,0))) == NULL) goto end_argv;
   switch (argc) {
   case 1:  result = ast_operator1(name,flags,argv[0]); break;
   case 2:  result = ast_operator2(name,flags,argv[0],argv[1]); break;
   case 3:  result = ast_operator3(name,flags,argv[0],argv[1],argv[2]); break;
...
do_generic:
 ...
 if (name >= AST_OPERATOR_MIN &&
  ...
  Dee_Incref(function_symbol->sym_extern.sym_module);
  function_symbol->sym_extern.sym_modsym = DeeModule_GetSymbolString(function_symbol->sym_extern.sym_module,
...
ast_parse_operator_name(uint16_t features) {
 ...
 case KWD_repr:          result = OPERATOR_REPR; goto done_y1;
 ...
 case KWD_copy:          result = OPERATOR_COPY; goto done_y1;
 case KWD_deepcopy:      result = OPERATOR_DEEPCOPY; goto done_y1;
 case KWD_super:
 ...
 case '=':
  ...
  if (TPP_ISKEYWORD(tok) && token.t_kwd->k_size == 4 &&
    *(uint32_t *)token.t_kwd->k_name == ENCODE4('m','o','v','e')) {
 ...
 case '(':
  ...
  if unlikely(result < 0) goto err_flags;
  TPPLexer_Current->l_flags |= old_flags & TPPLEXER_FLAG_WANTLF;
  if unlikely(tok != ')') {
 ...
 case '[':
  ...
  if unlikely(likely(tok == ']') ? (yield() < 0) :
              WARN(W_EXPECTED_RBRACKET_AFTER_LBRACKET))
     goto err;
  if (tok == '=') {
   if unlikely(yield() < 0) goto err;
   result = (result == AST_OPERATOR_GETITEM_OR_SETITEM
           ? OPERATOR_SETITEM : OPERATOR_SETRANGE);
  }
  goto done;
 ...
 default:
default_case:
  if (!TPP_ISKEYWORD(tok)) goto unknown;
  ...
  case 4:
   ...
   if (name == ENCODE4('f','o','r',0) && (features&P_OPERATOR_FCLASS)) { result = AST_OPERATOR_FOR; goto done_y1; }
  ...
  case 5:
   ...
   break;
  case 8:
   if (*((uint32_t *)(name_begin + 0)) == ENCODE4('c','o','n','t') &&
       *((uint32_t *)(name_begin + 4)) == ENCODE4('a','i','n','s'))
   { result = OPERATOR_CONTAINS; goto done_y1; }
   break;
  case 10:
   if (*((uint32_t *)(name_begin + 0)) == ENCODE4('d','e','s','t') &&
       *((uint32_t *)(name_begin + 4)) == ENCODE4('r','u','c','t') &&
       *((uint16_t *)(name_begin + 8)) == ENCODE2('o','r'))
   { result = OPERATOR_DESTRUCTOR; goto done_y1; }
   break;
  case 11:
   if (*((uint32_t *)(name_begin + 0)) == ENCODE4('c','o','n','s') &&
       *((uint32_t *)(name_begin + 4)) == ENCODE4('t','r','u','c') &&
       *((uint32_t *)(name_begin + 8)) == ENCODE4('t','o','r',0))
  ...
  if (name_size == 4 &&
    *(uint32_t *)(name_begin + 0) == ENCODE4('b','o','o','l'))
  { result = OPERATOR_BOOL; goto done_y1; }
  if (name_size == 3 &&
    *(uint32_t *)(name_begin + 0) == ENCODE4('i','n','t',0))
  { result = OPERATOR_INT; goto done_y1; }
  if (name_size == 5 &&
    *(uint32_t *)(name_begin + 0) == ENCODE4('f','l','o','a') &&
  ...
  { struct opinfo *info;
    ...
    if (name_size == 2) {
     if (*(uint16_t *)(name_begin + 0) == ENCODE2('l','t'))
         goto do_operator_lo;
     if (*(uint16_t *)(name_begin + 0) == ENCODE2('g','t'))
         goto do_operator_gr;
    }
    if (name_size == 6 &&
      *(uint32_t *)(name_begin + 0) == ENCODE4('r','e','a','d') &&
      *(uint16_t *)(name_begin + 4) == ENCODE2('n','p'))
    { result = FILE_OPERATOR_READ; goto done_y1; }
    if (name_size == 7 &&
      *(uint32_t *)(name_begin + 0) == ENCODE4('w','r','i','t') &&
      *(uint32_t *)(name_begin + 4) == ENCODE4('e','n','p',0))
    { result = FILE_OPERATOR_WRITE; goto done_y1; }
    if (name_size == 9 &&
      *(uint32_t *)(name_begin + 0) == ENCODE4('s','u','p','e') &&
      *(uint32_t *)(name_begin + 4) == ENCODE4('r','a','r','g') &&
      *(uint8_t  *)(name_begin + 8) == 's' && (features&P_OPERATOR_FCLASS))
    { result = CLASS_OPERATOR_SUPERARGS; goto done_y1; }
    if (name_size == 6 &&
      *(uint32_t *)(name_begin + 0) == ENCODE4('d','o','u','b') &&
      *(uint16_t *)(name_begin + 4) == ENCODE2('l','e'))
    { result = OPERATOR_FLOAT; goto done_y1; }
    if (name_size == 5 &&
       ((*(uint32_t *)(name_begin + 0) == ENCODE4('i','n','t','3') &&
         *(uint8_t  *)(name_begin + 4) == '2') ||
        (*(uint32_t *)(name_begin + 0) == ENCODE4('i','n','t','6') &&
