...
INTERN int DCALL skip_lf(void) {
 if (tok == '\n') {
  uint32_t old_flags; tok_t error;
  old_flags = TPPLexer_Current->l_flags;
  TPPLexer_Current->l_flags &= ~TPPLEXER_FLAG_WANTLF;
  error = yield();
  TPPLexer_Current->l_flags |= old_flags & TPPLEXER_FLAG_WANTLF;
...
ast_parse_for_head(DREF struct ast **__restrict pinit,
 ...
 if unlikely(likely(tok == ';') ? (yield() < 0) :
             WARN(W_EXPECTED_SEMICOLLON2_AFTER_FOR))
    goto err;
 if (tok == ')') {
  iter_or_next = NULL;
 } else {
...
do_realloc:
  ...
  if (token_num == token.t_num) {
   if unlikely(WARN(W_FAILED_TO_PARSE_STATEMENT)) goto err;
   if unlikely(yield() < 0) goto err;
  }
 }
...
again:
 ...
 case ';':
  ...
  if unlikely(yieldnbif(allow_nonblock) < 0) goto err;
  break;
...
err_tt_branch:
  ...
  if (tok == KWD_else) {
   if unlikely(yield() < 0) goto err_tt_branch;
do_else_branch:
   ff_branch = ast_parse_statement(allow_nonblock);
   if unlikely(!ff_branch) goto err_tt_branch;
  }
 ...
 case KWD_yield:
  ...
  if (current_basescope->bs_cflags&BASESCOPE_FRETURN &&
      WARN(W_YIELD_AFTER_RETURN)) goto err;
  if unlikely(likely(is_semicollon()) ? (yield_semicollonnbif(allow_nonblock) < 0) :
 ...
 case KWD_for:
  ...
  if unlikely(type < 0) goto err_flags;
  TPPLexer_Current->l_flags |= old_flags & TPPLEXER_FLAG_WANTLF;
  ...
  if unlikely(!loop) goto err_loop;
...
err_foreach_elem:
 ...
 case KWD_goto:
  ...
  if (TPP_ISKEYWORD(tok)) {
   ...
   if unlikely(!goto_label) goto err;
   if unlikely(yield() < 0) goto err;
  } else {
 ...
 case KWD_switch:
  ...
  TPPLexer_Current->l_flags &= ~TPPLEXER_FLAG_WANTLF;
  if unlikely(likely(tok == '(') ? (yield() < 0) :
              WARN(W_EXPECTED_LPAREN_AFTER_SWITCH))
     goto err_flags;
  ...
  TPPLexer_Current->l_flags |= old_flags & TPPLEXER_FLAG_WANTLF;
  if unlikely(likely(tok == ')') ? (yield() < 0) :
              WARN(W_EXPECTED_RPAREN_AFTER_SWITCH))
     goto err_r;
  ...
  if unlikely(!switch_block) goto err_r_switch;
 ...
 default:
  if (TPP_ISKEYWORD(tok)) {
   ...
   if (*next_token == ':' &&
    ...
    if unlikely(yield() < 0) goto err; /* Label name */
    if unlikely(skip_lf()) goto err;
    if unlikely(yield() < 0) goto err; /* `:' token. */
handle_post_label:
    if unlikely(skip_lf()) goto err;
    if unlikely(tok == '}') {
    ...
    if (!ast_shared(result) &&
     ...
     if unlikely(!merge) { Dee_Free(elemv); goto err_label_ast; }
     result = merge;
    }
    break;
 case KWD_case:
 ...
 case KWD_default:
    ...
    if unlikely(yield() < 0) goto err;
    if unlikely(likely(tok == ':') ? (yield() < 0) :
                WARN(W_EXPECTED_COLLON_AFTER_DEFAULT))
       goto err;
    if unlikely(!(current_basescope->bs_cflags&BASESCOPE_FSWITCH))
       goto again;
