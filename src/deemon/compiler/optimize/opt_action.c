...
PRIVATE bool DCALL
ast_sequence_is_nonempty(DeeAstObject *__restrict self) {
 (void)self;
 /* TODO: Check if `self' is a non-empty sequence expression. */
 return false;
}
PRIVATE bool DCALL
ast_iterator_is_nonempty(DeeAstObject *__restrict self) {
 /* Simple (and most likely) case: Check if `self' invokes `operator iter'. */
 if (self->ast_type == AST_OPERATOR &&
     self->ast_flag == OPERATOR_ITERSELF &&
   !(self->ast_operator.ast_exflag & (AST_OPERATOR_FPOSTOP|AST_OPERATOR_FVARARGS)) &&
     self->ast_operator.ast_opa)
     return ast_sequence_is_nonempty(self->ast_operator.ast_opa);
...
(DCALL ast_optimize_action)(struct ast_optimize_stack *__restrict stack,
 ...
 case AST_FACTION_STORE:
  ...
  if (self->ast_action.ast_act0->ast_type == AST_CONSTEXPR &&
   ...
   if (ast_optimize(stack,self->ast_action.ast_act1,result_used)) goto err;
  ...
  if (self->ast_action.ast_act0->ast_type == AST_SYM) {
   ...
   target_sym = self->ast_action.ast_act0->ast_sym;
...
action_set_expr_result:
 ...
 default:
  ...
  case 3:  if (ast_optimize(stack,self->ast_action.ast_act2,true)) goto err; ATTR_FALLTHROUGH
  case 2:  if (ast_optimize(stack,self->ast_action.ast_act1,true)) goto err; ATTR_FALLTHROUGH
  case 1:  if (ast_optimize(stack,self->ast_action.ast_act0,true)) goto err; ATTR_FALLTHROUGH
  default: break;
  }
  break;
 }
