...
(DCALL ast_optimize_action)(struct ast_optimize_stack *__restrict stack,
 ...
 case AST_FACTION_STORE:
  ...
  if (self->a_action.a_act0->a_type == AST_CONSTEXPR &&
   ...
   if (ast_optimize(stack,self->a_action.a_act1,result_used)) goto err;
...
action_set_expr_result:
 ...
 default:
  ...
  case 3:  if (ast_optimize(stack,self->a_action.a_act2,true)) goto err; ATTR_FALLTHROUGH
  case 2:  if (ast_optimize(stack,self->a_action.a_act1,true)) goto err; ATTR_FALLTHROUGH
  case 1:  if (ast_optimize(stack,self->a_action.a_act0,true)) goto err; ATTR_FALLTHROUGH
  default: break;
  }
  break;
 }
