...
(DCALL ast_assumes_rehashsymbol)(struct ast_assumes *__restrict self) {
 ...
 if unlikely(new_mask == 1) new_mask = 16-1;
 ...
 if unlikely(!new_vector) return -1;
 if (self->aa_syms.sa_elem) {
  ...
  for (; iter != end; ++iter) {
   ...
   for (;; i = AST_SYMBOL_ASSUMES_HASHNX(i,perturb),
    ...
    if (!item->sa_sym) break; /* Empty slot found. */
   }
...
ast_assumes_getsymbol(struct ast_assumes *__restrict self,
 ...
 if (!self->aa_syms.sa_elem) return NULL;
 i = perturb = hash & self->aa_syms.sa_mask;
 for (;; i = AST_SYMBOL_ASSUMES_HASHNX(i,perturb),
             AST_SYMBOL_ASSUMES_HASHPT(perturb)) {
  struct ast_symbol_assume *item;
  item = AST_SYMBOL_ASSUMES_HASHIT(&self->aa_syms,i);
  if (!item->sa_sym) break;
  if (item->sa_sym == sym) return item;
 }
 DEE_CHECKMEMORY();
 return NULL;
}
...
(DCALL ast_assumes_setsymval)(struct ast_assumes *__restrict self,
 ...
 if (!SYMBOL_ALLOW_ASSUMPTIONS(sym)) goto done;
 assumption = ast_assumes_newsymbol(self,sym);
 if unlikely(!assumption) goto err;
...
(DCALL ast_assumes_undefined)(struct ast_assumes *__restrict self) {
 ...
 if (self->aa_syms.sa_elem) {
  for (i = 0; i <= self->aa_syms.sa_mask; ++i) {
   if (!self->aa_syms.sa_elem[i].sa_sym) continue;
...
same_constant_value(DeeObject *__restrict a,
 ...
 int temp;
 if (a == b) return true;
 if (Dee_TYPE(a) != Dee_TYPE(b)) return false;
...
(DCALL ast_assumes_mergecond)(struct ast_assumes *__restrict child,
 ...
 for (i = 0; i <= child->aa_syms.sa_mask; ++i) {
  ...
  if (!cass->sa_sym) continue;
  if (!cass->sa_value) continue; /* Negative assumptions are or'd together. */
 ...
 for (i = 0; i <= sibling->aa_syms.sa_mask; ++i) {
  ...
  if (!sass->sa_sym) continue;
  if (sass->sa_value) continue; /* Positive assumptions have already been merged. */
  sass = ast_assumes_newsymbol(child,sass->sa_sym);
  if unlikely(!sass) goto err;
  sass->sa_value = NULL;
 }
 return 0;
err:
