...
f_libfs_expand(size_t argc, DeeObject **__restrict argv) {
 ...
 if (options_ob) {
  if (DeeString_Check(options_ob)) {
   ...
   /* `expand(string path, sequence env_mapping)' */
   env_ob = options_ob;
  } else {
   /* `expand(string path, int options, sequence env_mapping = environ)' */
...
for (local x: names) {
    print "#if STAT_"+x+" > UINT15_MAX";
    print "PRIVATE DEFINE_UINT16(libfs_S_"+x+",STAT_"+x+");";
    print "#else";
    print "PRIVATE DEFINE_UINT15(libfs_S_"+x+",STAT_"+x+");";
    print "#endif";
}
]]]*/
#if STAT_IFMT > UINT15_MAX
PRIVATE DEFINE_UINT16(libfs_S_IFMT,STAT_IFMT);
#else
PRIVATE DEFINE_UINT15(libfs_S_IFMT,STAT_IFMT);
#endif
#if STAT_IFDIR > UINT15_MAX
PRIVATE DEFINE_UINT16(libfs_S_IFDIR,STAT_IFDIR);
#else
PRIVATE DEFINE_UINT15(libfs_S_IFDIR,STAT_IFDIR);
#endif
#if STAT_IFCHR > UINT15_MAX
PRIVATE DEFINE_UINT16(libfs_S_IFCHR,STAT_IFCHR);
#else
PRIVATE DEFINE_UINT15(libfs_S_IFCHR,STAT_IFCHR);
#endif
#if STAT_IFBLK > UINT15_MAX
PRIVATE DEFINE_UINT16(libfs_S_IFBLK,STAT_IFBLK);
#else
PRIVATE DEFINE_UINT15(libfs_S_IFBLK,STAT_IFBLK);
#endif
#if STAT_IFREG > UINT15_MAX
PRIVATE DEFINE_UINT16(libfs_S_IFREG,STAT_IFREG);
#else
PRIVATE DEFINE_UINT15(libfs_S_IFREG,STAT_IFREG);
#endif
#if STAT_IFIFO > UINT15_MAX
PRIVATE DEFINE_UINT16(libfs_S_IFIFO,STAT_IFIFO);
#else
PRIVATE DEFINE_UINT15(libfs_S_IFIFO,STAT_IFIFO);
#endif
#if STAT_IFLNK > UINT15_MAX
PRIVATE DEFINE_UINT16(libfs_S_IFLNK,STAT_IFLNK);
#else
PRIVATE DEFINE_UINT15(libfs_S_IFLNK,STAT_IFLNK);
#endif
#if STAT_IFSOCK > UINT15_MAX
PRIVATE DEFINE_UINT16(libfs_S_IFSOCK,STAT_IFSOCK);
#else
PRIVATE DEFINE_UINT15(libfs_S_IFSOCK,STAT_IFSOCK);
#endif
#if STAT_ISUID > UINT15_MAX
PRIVATE DEFINE_UINT16(libfs_S_ISUID,STAT_ISUID);
#else
PRIVATE DEFINE_UINT15(libfs_S_ISUID,STAT_ISUID);
#endif
#if STAT_ISGID > UINT15_MAX
PRIVATE DEFINE_UINT16(libfs_S_ISGID,STAT_ISGID);
#else
PRIVATE DEFINE_UINT15(libfs_S_ISGID,STAT_ISGID);
#endif
#if STAT_ISVTX > UINT15_MAX
PRIVATE DEFINE_UINT16(libfs_S_ISVTX,STAT_ISVTX);
#else
PRIVATE DEFINE_UINT15(libfs_S_ISVTX,STAT_ISVTX);
#endif
#if STAT_IRUSR > UINT15_MAX
PRIVATE DEFINE_UINT16(libfs_S_IRUSR,STAT_IRUSR);
#else
PRIVATE DEFINE_UINT15(libfs_S_IRUSR,STAT_IRUSR);
#endif
#if STAT_IWUSR > UINT15_MAX
PRIVATE DEFINE_UINT16(libfs_S_IWUSR,STAT_IWUSR);
#else
PRIVATE DEFINE_UINT15(libfs_S_IWUSR,STAT_IWUSR);
#endif
#if STAT_IXUSR > UINT15_MAX
PRIVATE DEFINE_UINT16(libfs_S_IXUSR,STAT_IXUSR);
#else
PRIVATE DEFINE_UINT15(libfs_S_IXUSR,STAT_IXUSR);
#endif
#if STAT_IRGRP > UINT15_MAX
PRIVATE DEFINE_UINT16(libfs_S_IRGRP,STAT_IRGRP);
#else
PRIVATE DEFINE_UINT15(libfs_S_IRGRP,STAT_IRGRP);
#endif
#if STAT_IWGRP > UINT15_MAX
PRIVATE DEFINE_UINT16(libfs_S_IWGRP,STAT_IWGRP);
#else
PRIVATE DEFINE_UINT15(libfs_S_IWGRP,STAT_IWGRP);
#endif
#if STAT_IXGRP > UINT15_MAX
PRIVATE DEFINE_UINT16(libfs_S_IXGRP,STAT_IXGRP);
#else
PRIVATE DEFINE_UINT15(libfs_S_IXGRP,STAT_IXGRP);
#endif
#if STAT_IROTH > UINT15_MAX
PRIVATE DEFINE_UINT16(libfs_S_IROTH,STAT_IROTH);
#else
PRIVATE DEFINE_UINT15(libfs_S_IROTH,STAT_IROTH);
#endif
#if STAT_IWOTH > UINT15_MAX
PRIVATE DEFINE_UINT16(libfs_S_IWOTH,STAT_IWOTH);
#else
PRIVATE DEFINE_UINT15(libfs_S_IWOTH,STAT_IWOTH);
#endif
#if STAT_IXOTH > UINT15_MAX
PRIVATE DEFINE_UINT16(libfs_S_IXOTH,STAT_IXOTH);
#else
...
PRIVATE struct dex_symbol symbols[] = {
    ...
    { "chdir", (DeeObject *)&libfs_chdir, MODSYM_FNORMAL,
      DOC("(string path)->none\n"
          ...
          "@throw HandleClosed The given @fp or @fd has been closed or is invalid\n"
          "@throw AccessError The current user does not have permissions to enter @path\n"
    ...
    { "chtime", (DeeObject *)&libfs_chtime, MODSYM_FNORMAL,
      DOC("(string path,time atime,time mtime,time ctime=none)->none\n"
          ...
          "@throw HandleClosed The given file @fp was closed, or the given handle @fd is invalid\n"
          "@throw AccessError The current user does not have permissions to "
          ...
          "@throw ReadOnly The filesystem or device hosting the file found under "
    ...
    { "chmod", (DeeObject *)&libfs_chmod, MODSYM_FNORMAL,
      DOC("(string path,string mode)->none\n"
          ...
          "@throw NoDirectory A part of the given @path is not a directory\n"
          "@throw HandleClosed The given file @fp was closed, or the given handle @fd is invalid\n"
          "@throw AccessError The current user does not have permissions "
    ...
    { "mkdir", (DeeObject *)&libfs_mkdir, MODSYM_FNORMAL,
      DOC("(string path,string permissions=none)->none\n"
          ...
          "@throw ReadOnly The filesystem or device hosting the directory of "
                          "@path is in read-only operations mode, preventing the "
    ...
    { "rename", (DeeObject *)&libfs_rename, MODSYM_FNORMAL,
      DOC("(string existing_path,string new_path)->none\n"
          ...
          "@throw ReadOnly The filesystem or device hosting the given file or directory "
    ...
    { "copyfile", (DeeObject *)&libfs_copyfile, MODSYM_FNORMAL,
      DOC("(string existing_file,string new_file,callable progress=none)->none\n"
          ...
          "@throw AccessError The current user does not have permissions to access "
                             "the file @existing_file for reading, or the directory "
                             "containing the non-existant file @new_file for writing\n"
          "@throw ReadOnly The filesystem or device hosting the directory "
    ...
    { "abspath", (DeeObject *)&libfs_abspath, MODSYM_FNORMAL,
      DOC("(string path,string cwd=\".\")->string\n"
          "@interrupt\n"
    ...
    { "S_ISBLK",  (DeeObject *)&libfs_S_ISBLK,  MODSYM_FREADONLY|MODSYM_FCONSTEXPR, DOC("(int mode)->bool") },
    { "S_ISREG",  (DeeObject *)&libfs_S_ISREG,  MODSYM_FREADONLY|MODSYM_FCONSTEXPR, DOC("(int mode)->bool") },
    { "S_ISFIFO", (DeeObject *)&libfs_S_ISFIFO, MODSYM_FREADONLY|MODSYM_FCONSTEXPR, DOC("(int mode)->bool") },
    { "S_ISLNK",  (DeeObject *)&libfs_S_ISLNK,  MODSYM_FREADONLY|MODSYM_FCONSTEXPR, DOC("(int mode)->bool") },
    { "S_ISSOCK", (DeeObject *)&libfs_S_ISSOCK, MODSYM_FREADONLY|MODSYM_FCONSTEXPR, DOC("(int mode)->bool") },
