...
typedef WUNUSED NONNULL((1, 3)) uint64_t (DCALL *dhashfunc64_t)(struct dhashalgo64 const *__restrict self, uint64_t start, void const *__restrict data, size_t datasize);

/* Hash function prototype for extended-width hash functions. (with hash values of up to 256 bytes) */
typedef WUNUSED NONNULL((1, 2, 3)) DREF DeeObject *
