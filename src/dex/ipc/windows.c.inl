...
print_environ(struct ascii_printer *__restrict printer,
 ...
 while (ITER_ISOK(item = DeeObject_IterNext(iter))) {
  if (DeeObject_Unpack(item,2,name_and_value)) goto err_item;
...
get_stdhandle_for_process(DeeObject *procfd, int stdnum) {
 HANDLE result;
 result = procfd ? DeeFile_Fileno(procfd) :
...
nt_GetEnvironmentVariable(LPCWSTR lpName) {
 LPWSTR buffer; DWORD bufsize = 256,error;
 LPWSTR new_buffer;
...
nt_CreateProcessPathNoExt(LPWSTR lpApplicationName, SIZE_T szApplicationNameLength,
 ...
 for (;;) {
  ...
  if (bFixUnc) {
   LPWSTR pathname; DREF DeeStringObject *result,*fixed_result;
   ...
   if unlikely(!fixed_result) { err_result: Dee_DecrefDokill(result); goto err; }
...
again:
 result = nt_CreateProcessPathNoExt(lpApplicationName,szApplicationNameLength,pathStr,
                                    ...
                                    lpCurrentDirectory,lpStartupInfo,lpProcessInformation);
 if (result != (DREF DeeStringObject *)ITER_DONE) goto done;
 ...
 if (pathext != (DREF DeeStringObject *)ITER_DONE) {
  ...
  if unlikely(!pathext) goto err;
  appnameBuffer = (LPWSTR)Dee_Malloc((appnameBufferSize+1)*sizeof(WCHAR));
  if unlikely(!appnameBuffer) goto err_pathext;
  iter = (LPWSTR)DeeString_AsWide((DeeObject *)pathext);
  if unlikely(!iter) { Dee_Free(appnameBuffer); goto err_pathext; }
  ...
  for (;;) {
   ...
   if unlikely(appnameLength > appnameBufferSize) {
    ...
    if unlikely(!newAppnameBuffer) { Dee_Free(appnameBuffer); goto err_pathext; }
 ...
 if (!bFixUnc) { bFixUnc = TRUE; goto again; }
done:
 Dee_Decref(path);
done_nopath:
 return result;
...
process_start(Process *__restrict self,
 ...
 DeeObject *result = NULL; size_t wCmdLineSize;
...
again:
 ...
 if unlikely(!wExe) goto done_procenv;
 wCmdLine = (LPWSTR)DeeString_AsWide((DeeObject *)cmdline);
 if unlikely(!wCmdLine) goto done_procenv;
 ...
 if unlikely(!wCmdLineCopy) goto done_procenv;
 ...
 if unlikely(startupInfo.hStdInput == INVALID_HANDLE_VALUE) goto done_cmdline_copy;
 startupInfo.hStdOutput = get_stdhandle_for_process(procout,DEE_STDOUT);
 if unlikely(startupInfo.hStdOutput == INVALID_HANDLE_VALUE) goto done_cmdline_copy;
 startupInfo.hStdError  = get_stdhandle_for_process(procerr,DEE_STDERR);
 if unlikely(startupInfo.hStdError == INVALID_HANDLE_VALUE) goto done_cmdline_copy;
...
again:
 if (!(self->p_state&PROCESS_FDIDJOIN)) {
  ...
  while (ATOMIC_FETCHOR(self->p_state,PROCESS_FDETACHING) &
   ...
   if (timeout_microseconds == 0) return 1; /* Timeout */
   if (DeeThread_Sleep(1000)) goto err; /* Error (interrupt delivery) */
   now = DeeThread_GetTimeMicroSeconds();
   if (now >= timeout_end) return 1; /* Timeout */
...
done_join:
  ...
  do state = ATOMIC_READ(self->p_state);
...
process_set_pwd(Process *__restrict self, DeeObject *value) {
 ...
 if (self == &this_process) {
  DREF DeeObject *temp;
  if unlikely(!value) goto err_running;
...
err:
 ...
 rwlock_endwrite(&self->p_lock);
 if (old_value) Dee_Decref(old_value);
 else if (!value) {
  err_unbound_attribute(&DeeProcess_Type,"exe");
  goto err;
 }
...
process_get_argv(Process *__restrict self) {
 DREF DeeObject *result; DREF DeeStringObject *cmdline;
...
process_set_argv(Process *__restrict self, DeeObject *__restrict value) {
 DREF DeeStringObject *cmdline; int result;
 struct ascii_printer printer = ASCII_PRINTER_INIT;
...
process_init(Process *__restrict self,
 ...
 if (DeeString_Check(exe)) {
  if unlikely(cmdline_add_arg(&cmdline,exe)) goto err;
  Dee_Incref(exe);
 } else {
  if (DeeInt_Check(exe)) {
   HANDLE fd;
  ...
  if unlikely(!exe) goto err;
  temp = cmdline_add_arg(&cmdline,exe);
  if unlikely(temp) goto err_exe;
 }
...
INTERN DeeTypeObject DeeProcess_Type = {
    OBJECT_HEAD_INIT(&DeeType_Type),
    /* .tp_name     = */"process",
    /* .tp_doc      = */DOC("(string exe)\n"
                            "(string exe,sequence args)\n"
