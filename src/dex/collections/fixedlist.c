...
#include <deemon/object.h>          /* DREF, DeeObject, DeeObject_*, DeeTypeObject, Dee_AsObject, Dee_BOUND_*, Dee_COMPARE_*, Dee_Decref, Dee_Decrefv, Dee_HAS_ISERR, Dee_HAS_ISYES_NO_ERR, Dee_Incref, Dee_Incref_n, Dee_WEAKREF_SUPPORT_ADDR, Dee_XDecref*, Dee_XIncref, Dee_XMovrefv, Dee_hash_t, Dee_return_compareT, Dee_ssize_t, Dee_weakref_support_fini, Dee_weakref_support_init, ITER_DONE, OBJECT_HEAD_INIT, return_reference_ */
...
fl_getitem_index(FixedList *__restrict self, size_t index) {
	...
	if unlikely(index >= self->fl_size) {
		DeeRT_ErrIndexOutOfBounds(Dee_AsObject(self), index, self->fl_size);
		goto err;
	}
	FixedList_LockRead(self);
