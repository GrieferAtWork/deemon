...
fl_copy(FixedList *__restrict self) {
 DREF FixedList *result; size_t i;
 result = (DREF FixedList *)DeeGCObject_Malloc(offsetof(FixedList,fl_elem) +
...
fl_init_getitem(DREF DeeObject *(DCALL *getitem)(DeeObject *__restrict self,
 ...
 DREF FixedList *result;
 size_t i; DREF DeeObject *index_ob,*elem;
 result = (DREF FixedList *)DeeGCObject_Malloc(offsetof(FixedList,fl_elem) +
                                              (length * sizeof(DREF DeeObject *)));
 if unlikely(!result) goto err;
 for (i = 0; i < length; ++i) {
  index_ob = DeeInt_NewSize(i);
  if unlikely(!index_ob) goto err_r;
...
fl_assign(FixedList *__restrict self, DeeObject *__restrict other) {
 DREF DeeObject **items,*temp; size_t i;
...
fli_init(FixedListIterator *__restrict self,
         size_t argc, DeeObject **__restrict argv) {
 self->li_iter = 0;
 if (DeeArg_Unpack(argc,argv,"o|" DEE_FMT_SIZE_T ":FixedListIterator",&self->li_list,&self->li_iter))
     goto err;
...
PRIVATE struct type_cmp fli_cmp = {
    /* .tp_hash = */NULL,
    /* .tp_eq   = */(DREF DeeObject *(DCALL *)(DeeObject *__restrict self, DeeObject *__restrict some_object))&fli_eq,
    /* .tp_ne   = */(DREF DeeObject *(DCALL *)(DeeObject *__restrict self, DeeObject *__restrict some_object))&fli_ne,
    /* .tp_lo   = */(DREF DeeObject *(DCALL *)(DeeObject *__restrict self, DeeObject *__restrict some_object))&fli_lo,
    /* .tp_le   = */(DREF DeeObject *(DCALL *)(DeeObject *__restrict self, DeeObject *__restrict some_object))&fli_le,
    /* .tp_gr   = */(DREF DeeObject *(DCALL *)(DeeObject *__restrict self, DeeObject *__restrict some_object))&fli_gr,
    /* .tp_ge   = */(DREF DeeObject *(DCALL *)(DeeObject *__restrict self, DeeObject *__restrict some_object))&fli_ge,
};
