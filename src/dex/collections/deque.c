...
Deque_PushFront_unlocked(Deque *__restrict self,
 ...
 if unlikely(!new_bucket) return false;
...
Deque_llrot_unlocked(Deque *__restrict self, size_t num_objects) {
 DequeIterator iter; DREF DeeObject *lobj;
 if unlikely(num_objects <= 1) return;
...
Deque_lrrot_unlocked(Deque *__restrict self, size_t num_objects) {
 DequeIterator iter; DREF DeeObject *temp,*next;
 if unlikely(num_objects <= 1) return;
 ASSERT(num_objects <= self->d_size);
 DequeIterator_InitBegin(&iter,self);
 num_objects -= 2;
...
Deque_rrrot_unlocked(Deque *__restrict self, size_t num_objects) {
 DequeIterator iter; DREF DeeObject *robj;
 if unlikely(num_objects <= 1) return;
 ASSERT(num_objects <= self->d_size);
 DequeIterator_InitRBegin(&iter,self);
 num_objects -= 2;
...
again:
 ...
 if (other->d_head) {
  ...
  copy = copy_tail = copy_bucket(iter,
                                 ...
                                 other->d_head_idx,
                                 other->d_head_use);
  if unlikely(!copy) goto err_restart_collect;
  ...
  while (iter != other->d_tail) {
   ...
   if unlikely(!next) goto err_restart_collect;
...
err_restart_collect:
 ...
 if (iter) for (;;) {
  ...
  else i = 0,count = self->d_bucket_sz;
  while (count--) {
   Dee_Decref(iter->db_items[i]);
   ++i;
  }
  Dee_Free(iter);
  if (iter == copy_tail) break;
  iter = next;
 }
...
deq_deepload(Deque *__restrict self) {
 Deque_LockRead(self);
 if (self->d_size) {
  DequeIterator iter; size_t version;
...
deq_erase(Deque *__restrict self, size_t argc,
 ...
 if (DeeArg_UnpackKw(argc,argv,kw,seq_erase_kwlist,"Id|" DEE_FMT_SIZE_T ":erase",&index,&num_items))
     goto err;
...
deq_llrot(Deque *__restrict self,
          size_t argc, DeeObject **__restrict argv) {
 size_t num_items;
 if (DeeArg_Unpack(argc,argv,DEE_FMT_SIZE_T ":llrot",&num_items) ||
...
deq_lrrot(Deque *__restrict self,
          size_t argc, DeeObject **__restrict argv) {
 size_t num_items;
 if (DeeArg_Unpack(argc,argv,DEE_FMT_SIZE_T ":lrrot",&num_items) ||
...
deq_rlrot(Deque *__restrict self,
          size_t argc, DeeObject **__restrict argv) {
 size_t num_items;
 if (DeeArg_Unpack(argc,argv,DEE_FMT_SIZE_T ":rlrot",&num_items) ||
...
deq_rrrot(Deque *__restrict self,
          size_t argc, DeeObject **__restrict argv) {
 size_t num_items;
 if (DeeArg_Unpack(argc,argv,DEE_FMT_SIZE_T ":rrrot",&num_items) ||
...
deqiter_ctor(DequeIteratorObject *__restrict self) {
 ...
 if unlikely(!self->di_deq) return -1;
 self->di_ver = 0;
