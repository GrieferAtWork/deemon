...
#include <deemon/type.h>            /* DeeObject_InitStatic, Dee_TYPE_CONSTRUCTOR_INIT_FIXED, INT_SIGNED, INT_UNSIGNED, TF_NONE, TP_F*, TYPE_MEMBER_CONST, TYPE_MEMBER_END, type_member */
...
INTERN WUNUSED DREF DeeObject *DCALL int_newint(CTYPES_INT val) {
	...
	if unlikely(!result)
		goto done;
	DeeObject_InitStatic(result, DeeSType_AsType(&DeeCInt_Type));
	result->i_value = val;
done:
