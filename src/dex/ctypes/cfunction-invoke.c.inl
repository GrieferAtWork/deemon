...
#ifdef VARARGS
 size_t n_varargs;
 DeeTypeObject *dee_va_type; DeeObject *dee_va_arg;
...
#define argt  tp_self->ft_argv[(size_t)(arg_iter-argv)]
   ...
   *argp_iter = (void *)iter;
   ...
   case FFI_TYPE_LONGDOUBLE:
    if (DeeObject_AsDouble(arg,&temp)) goto err_wbuf;
    iter->ld = (long double)temp;
   } break;
#endif
   case FFI_TYPE_UINT8:  if (DeeObject_AsUInt8(arg,&iter->u8)) goto err_wbuf; break;
   case FFI_TYPE_SINT8:  if (DeeObject_AsInt8(arg,&iter->s8)) goto err_wbuf; break;
   case FFI_TYPE_UINT16: if (DeeObject_AsUInt16(arg,&iter->u16)) goto err_wbuf; break;
   case FFI_TYPE_SINT16: if (DeeObject_AsInt16(arg,&iter->s16)) goto err_wbuf; break;
   case FFI_TYPE_UINT32: if (DeeObject_AsUInt32(arg,&iter->u32)) goto err_wbuf; break;
   case FFI_TYPE_SINT32: if (DeeObject_AsInt32(arg,&iter->s32)) goto err_wbuf; break;
   case FFI_TYPE_UINT64: if (DeeObject_AsUInt64(arg,&iter->u64)) goto err_wbuf; break;
   case FFI_TYPE_SINT64: if (DeeObject_AsInt64(arg,&iter->s64)) goto err_wbuf; break;
 ...
 if (n_varargs) {
  ...
  for (;;) {
...
#if __SIZEOF_INT__ == 4
    if (DeeInt_TryGetS32(dee_va_arg,&iter->s32))
     *dee_va_ffi_types = &ffi_type_sint;
    else {
     if (DeeInt_GetU32(dee_va_arg,&iter->u32))
         goto err_wbuf;
     *dee_va_ffi_types = &ffi_type_uint;
    }
#elif __SIZEOF_INT__ == 8
    if (DeeInt_TryGetS64(dee_va_arg,&iter->s64)) {
     *dee_va_ffi_types = &ffi_type_sint;
    } else {
     if (DeeInt_GetU64(dee_va_arg,&iter->u64))
         goto err_wbuf;
     *dee_va_ffi_types = &ffi_type_uint;
    }
#elif __SIZEOF_INT__ <= 4
    int32_t temp;
    if (DeeInt_TryGetS32(dee_va_arg,&temp)) {
     iter->i = (int)temp;
     *dee_va_ffi_types = &ffi_type_sint;
    } else {
     if (DeeInt_TryGetU32(dee_va_arg,(uint32_t *)&temp))
         goto err_wbuf;
    ...
    *dee_va_ffi_types = stype_ffitype((DeeSTypeObject *)dee_va_type);
    ...
    case 1: iter->u8  = *(uint8_t *)DeeStruct_Data(dee_va_arg); goto def_var_data;
    case 2: iter->u16 = *(uint16_t *)DeeStruct_Data(dee_va_arg); goto def_var_data;
    case 4: iter->u32 = *(uint32_t *)DeeStruct_Data(dee_va_arg);
def_var_data: *argp_iter = (void *)iter; break;
    case 8: iter->u64 = *(uint64_t *)DeeStruct_Data(dee_va_arg); goto def_var_data;
    default:
...
#undef ret_mem
done_wbuf: Dee_AFree(wbuf);
done:      return result;
err_wbuf: result = NULL; goto done_wbuf;
