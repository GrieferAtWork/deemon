...
cfunctiontype_new(DeeSTypeObject *__restrict return_type,
 ...
 if (inherit_argv) {
  ...
  if unlikely(!argv_copy) goto err;
  for (i = 0; i < argc; ++i) {
 ...
 if unlikely(!result->ft_ffi_return_type) goto err_argv_r_name;
 result->ft_ffi_arg_type_v  = (ffi_type **)Dee_Malloc(argc*sizeof(ffi_type *));
 if unlikely(!result->ft_ffi_arg_type_v) goto err_argv_r_name;
 for (i = 0; i < argc; ++i) {
  ffi_type *tp = stype_ffitype(argv_copy[i]);
  if unlikely(!tp) goto err_argv_r_name_ffi_typev;
 ...
 if (!result->ft_wsize) result->ft_wsize = 1;
 if (!argc) result->ft_woff_argmem = 0,
...
again:
 ...
 if unlikely(!new_map) {
  ...
  if (!self->st_cfunction.sf_list && new_mask != 0) { new_mask = 1; goto again; }
  return false;
 }
 /* Do the re-hash. */
...
cfunction_equals(DeeCFunctionTypeObject *__restrict self,
                 ...
                 DeeSTypeObject **__restrict argv) {
 size_t i;
 if (self->ft_orig != return_type) goto nope;
 if (self->ft_argc != argc) goto nope;
 if (self->ft_cc   != calling_convention) goto nope;
 for (i = 0; i < argc; ++i) {
  if (self->ft_argv[i] != argv[i]) goto nope;
 }
 return true;
nope:
 return false;
...
register_type:
 ...
 if (return_type->st_cfunction.sf_size >= return_type->st_cfunction.sf_mask &&
    (!stype_cfunction_rehash(return_type,return_type->st_cfunction.sf_mask ?
                             (return_type->st_cfunction.sf_mask << 1)|1 : 16-1) &&
     !return_type->st_cfunction.sf_mask)) {
  /* No space at all! */
#ifndef CONFIG_NO_THREADS
