...
sock_getmsgflagsof(DeeObject *__restrict name,
 ...
 if (DeeString_Check(name)) {
  ...
  while (*iter) {
   ...
   for (;;) {
    if (MEMCASEEQ(iter,"MSG",3*sizeof(char))) { iter += 3; continue; }
    if (*iter == '_') { ++iter; continue; }
    break;
   }
   /* Search for the end of this flag. */
   { char *temp = strchr(iter,'|');
     if (!temp) temp = strchr(iter,',');
     if (!temp) temp = iter+strlen(iter);
   ...
   if (part_length < COMPILER_LENOF(sock_msg_names[0].md_name)) {
    for (desc  = sock_msg_names;
     ...
     if (!MEMCASEEQ(desc->md_name,iter,part_length)) continue;
     /* Found it! */
     result |= desc->md_flag;
     goto next_part;
    }
...
sock_getafname(int value) {
 ...
 for (; iter != COMPILER_ENDOF(sock_af_names); ++iter) {
  if (iter->ad_value != value) continue;
...
sock_gettypename(int value) {
 ...
 for (; iter != COMPILER_ENDOF(sock_type_names); ++iter) {
  if (iter->td_value != value) continue;
...
sock_getafvalue(char const *__restrict name, int *__restrict presult) {
 struct af_desc const *iter = sock_af_names;
 size_t name_len;
 if (MEMCASEEQ(name,"AF_",3*sizeof(char))) name += 3;
 name_len = strlen(name);
 for (; iter != COMPILER_ENDOF(sock_af_names); ++iter) {
  if (!MEMCASEEQ(iter->ad_name,name,name_len*sizeof(char))) continue;
...
sock_gettypevalue(char const *__restrict name, int *__restrict presult) {
 struct type_desc const *iter = sock_type_names;
 size_t name_len;
 if (MEMCASEEQ(name,"SOCK_",5*sizeof(char))) name += 5;
 name_len = strlen(name);
 for (; iter != COMPILER_ENDOF(sock_type_names); ++iter) {
  if (!MEMCASEEQ(iter->td_name,name,name_len*sizeof(char))) continue;
...
again:
 ...
 for (; *name; ++name) {
  if (!MEMCASEEQ(name,"PROTO",5*sizeof(char))) continue;
  name += 5;
  while (*name == '_') ++name;
  goto again;
 }
...
get_shutdown_mode(char const *__restrict mode,
                  int *__restrict presult) {
 char const *used_mode = mode; size_t mode_length;
 struct shutdown_option const *iter;
 for (;;) {
  if (MEMCASEEQ(used_mode,"SHUT",4*sizeof(char))) { used_mode += 4; continue; }
  if (*used_mode == '_') { ++used_mode; continue; }
  break;
 }
 iter = shutdown_options,mode_length = strlen(used_mode);
 for (; iter != COMPILER_ENDOF(shutdown_options); ++iter) {
  if (!MEMCASEEQ(iter->so_nam,mode,mode_length*sizeof(char))) continue;
  *presult = iter->so_opt;
  return 0;
 }
...
sock_gethostbyaddr(void const *__restrict data, socklen_t datalen,
 ...
 struct hostent *hp; DeeStringObject *result;
 neterrno_t error; size_t name_length;
#ifdef TRY_AGAIN
...
restart:
 ...
 if (!hp) {
  ...
  if (error == HOST_NOT_FOUND) {
   ...
   if (DeeThread_Sleep(10000)) goto err;
   ++attempt_counter;
   goto restart;
#endif
  } else {
 ...
 return (DREF DeeObject *)result;
err:    return NULL;
nodns2: rwlock_endwrite(&sysdb_lock);
nodns:
#ifdef AF_INET
 if (family == AF_INET) {
...
get_port_name(char const *__restrict port, size_t port_length,
 ...
 if unlikely(!port_length) goto invalid_port;
 do {
  char ch = *iter++;
...
SockAddr_FromStringPort(SockAddr *__restrict self, int family, int protocol, int type,
 ...
 while (host_length && DeeUni_IsSpace(*host)) ++host,--host_length;
 while (host_length && DeeUni_IsSpace(host[host_length-1])) --host_length;
 while (port_length && DeeUni_IsSpace(*port)) ++port,--port_length;
 while (port_length && DeeUni_IsSpace(port[port_length-1])) --port_length;
 ...
 if (host[host_length]) {
  ...
  if unlikely(!host_buffer) goto err;
 ...
 if (port[port_length]) {
  ...
  if unlikely(!port_buffer) goto err;
  memcpy(port_buffer,port,port_length*sizeof(char));
  port_buffer[port_length] = '\0';
  port = port_buffer;
 }
#ifdef AF_INET
 if (family == AF_INET) goto do_gethostbyname;
#endif
#ifdef AF_RDS
 if (family == AF_RDS) goto do_gethostbyname;
#endif
 {
  ...
  if (host_length && (!DeeUni_IsDecimal(*host) && *host != ':')) {
   ...
   if (special_family == AF_INET6 ||
    ...
    for (iter = speical_inet6_names;
     ...
     if (!MEMCASEEQ(iter->name,special_name,special_size)) continue;
...
SockAddr_FromString(SockAddr *__restrict self,
 ...
 char chr; char const *port_begin,*port_end;
...
#define __SIZEOF_PID_T__ 4
 ...
 case AF_BLUETOOTH: {
  ...
  case BTPROTO_L2CAP: {
   if unlikely(argc != 2) {
    if (argc == 1) goto do_generic_string;
    DeeError_Throwf(&DeeError_TypeError,
  ...
  case BTPROTO_HCI: {
   if unlikely(argc != 1) {
    if (argc == 2) goto do_generic_string_2;
    DeeError_Throwf(&DeeError_TypeError,
                    "Construction address Family AF_BLUETOOTH with protocol "
  ...
  case BTPROTO_SCO: {
   ...
   if (DeeObject_AssertTypeExact(argv[0],&DeeString_Type)) goto err;
   if unlikely(priv_stobdaddr(DeeString_STR(argv[0]),&self->bt_sco.sco_bdaddr)) goto err;
  } break;
#endif
