...
again:
 ...
 if (!item) {
  ...
  if (self->t_factory &&
   ...
   if unlikely(*pitem != NULL) { Dee_Decref(item); goto again; }
   *pitem = ITER_DONE;
   COMPILER_BARRIER();
   Dee_Decref(item);;
  } else {
...
again:
 ...
 if (!item) {
  ...
  if (self->t_factory &&
   ...
   if unlikely(!item) goto err;
   /* Check if the factory tinkered with the TLS variable. */
   if unlikely(self->t_value != NULL) { Dee_Decref(item); goto again; }
