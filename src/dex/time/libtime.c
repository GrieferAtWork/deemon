...
PUBLIC DREF DeeObject *(DeeTime_New)(uint64_t microseconds) {
...
get_repr_name(uint8_t repr) {
 ...
 for (iter  = repr_desc;
  ...
  if (iter->repr == repr) result = iter->name;
 }
 return result;
}
...
PRIVATE DREF DeeTimeObject *DCALL
...
time_getint(DeeTimeObject *__restrict self,
 ...
 if (self->t_type == TIME_MONTHS) {
  ...
  case TIME_REPR_MON:  result = self->t_months % MONTHS_PER_YEAR; goto done;
  case TIME_REPR_YER:  result = self->t_months / MONTHS_PER_YEAR; goto done;
  case TIME_REPR_DEC:  result = self->t_months / (MONTHS_PER_YEAR*YEARS_PER_DECADE); goto done;
  case TIME_REPR_CEN:  result = self->t_months / (MONTHS_PER_YEAR*YEARS_PER_DECADE*DECADES_PER_CENTURY); goto done;
  case TIME_REPR_MLL:  result = self->t_months / (MONTHS_PER_YEAR*YEARS_PER_DECADE*DECADES_PER_CENTURY*CENTURIES_PER_MILLENIUM); goto done;
  case TIME_REPR_MONS: result = self->t_months; goto done;
  default: break;
  }
 }
 msecs = DeeTime_Get(self);
 switch (repr) {
 case TIME_REPR_MIC:  result = time_get_microseconds(msecs) % MICROSECONDS_PER_MILLISECOND; goto done;
 case TIME_REPR_MIL:  result = time_get_milliseconds(msecs) % MILLISECONDS_PER_SECOND; goto done;
 case TIME_REPR_SEC:  result = time_get_seconds(msecs) % SECONDS_PER_MINUTE; goto done;
 case TIME_REPR_MIN:  result = time_get_minutes(msecs) % MINUTES_PER_HOUR; goto done;
 case TIME_REPR_HOR:  result = time_get_hours(msecs) % HOURS_PER_DAY; goto done;
 case TIME_REPR_WDAY: result = time_get_days(msecs) % DAYS_PER_WEEK; goto done;
 case TIME_REPR_MON:  result = time_get_months(msecs) % MONTHS_PER_YEAR; goto done;
 case TIME_REPR_YER:  result = time_get_years(msecs); goto done;
 case TIME_REPR_DEC:  result = time_get_decades(msecs); goto done;
 case TIME_REPR_CEN:  result = time_get_centuries(msecs); goto done;
 case TIME_REPR_MLL:  result = time_get_millenia(msecs); goto done;
 case TIME_REPR_YDAY:
 ...
 case TIME_REPR_MICS: result = time_get_microseconds(msecs); goto done;
 case TIME_REPR_MILS: result = time_get_milliseconds(msecs); goto done;
 case TIME_REPR_SECS: result = time_get_seconds(msecs); goto done;
 case TIME_REPR_MINS: result = time_get_minutes(msecs); goto done;
 case TIME_REPR_HORS: result = time_get_hours(msecs); goto done;
 case TIME_REPR_DAYS: result = time_get_days(msecs); goto done;
 case TIME_REPR_WEKS: result = time_get_weeks(msecs); goto done;
 case TIME_REPR_MONS: result = time_get_months(msecs); goto done;
...
time_setint(DeeTimeObject *__restrict self,
 ...
 case TIME_REPR_MIC:
  ...
  newval = ((newval / MICROSECONDS_PER_MILLISECOND) *
                      MICROSECONDS_PER_MILLISECOND)+value;
  break;
 case TIME_REPR_MIL:
 ...
 case TIME_REPR_MLL: arg = (dtime_half_t)value*YEARS_PER_MILLENIUM; goto do_set_year;
 case TIME_REPR_CEN: arg = (dtime_half_t)value*YEARS_PER_CENTURY; goto do_set_year;
 case TIME_REPR_DEC: arg = (dtime_half_t)value*YEARS_PER_DECADE; goto do_set_year;
 case TIME_REPR_YER: arg = (dtime_half_t)value;
do_set_year:
  if (self->t_type == TIME_MONTHS) {
...
#define print(p,s)  do{ if unlikely((temp = unicode_printer_print(printer,p,s)) < 0) goto err; result += temp; }__WHILE0
#define printf(...) do{ if unlikely((temp = unicode_printer_printf(printer,__VA_ARGS__)) < 0) goto err; result += temp; }__WHILE0
 unsigned int number;
 char const *text;
 ...
 * %W	Week number with the first Monday as the first day of week one (00-53)	34 */
   ...
   case '[': {
    ...
    unsigned int bracket_recursion = 1; dtime_t attribval;
    int repr_mode,width = 0; uint8_t attribute_id;
    /* Extended formatting */
    mode_end = mode_begin = tag_begin = format;
    for (;;) {
     ch = *format++;
     if (ch == ']') { if (!--bracket_recursion) { tag_end = format-1; break; } }
     else if (ch == '[') ++bracket_recursion;
     else if (ch == ':' && bracket_recursion == 1)
      mode_end = format-1,tag_begin = format;
     else if (!ch) { tag_end = format; break; }
    }
    ...
    switch (repr_mode) {
     ...
     default: {
      PRIVATE char const _suffix_values[] = "st" "nd" "rd" "th";
#if 0
      ...
      if (ascii_printer_SpecWriteUInt64(printer,attribval,&fmt_spec) < 0) goto err;
#else
...
PRIVATE bool DCALL determine_isdst(DeeTimeObject *__restrict self) {
 int previousSunday;
 unsigned int month;
...
time_timepart_set(DeeTimeObject *__restrict self,
 ...
 dtime_half_t addend;
 if (object_as_time_half(value,&addend)) goto err;
...
time_datepart_get(DeeTimeObject *__restrict self) {
 ...
 result->t_time = self->t_time;
 if (self->t_kind == TIME_MICROSECONDS) {
  result->t_time = (result->t_time / MICROSECONDS_PER_DAY)
...
time_datepart_set(DeeTimeObject *__restrict self,
 ...
 dtime_t tval = DeeTime_Get(self);
 dtime_t addend;
 if (object_as_time(value,&addend)) goto err;
...
time_div(DeeTimeObject *__restrict self,
 ...
 if (self->t_type == TIME_MICROSECONDS) {
  if (object_as_time(other,&result->t_time))
      goto err;
  if unlikely(!result->t_time) goto err_divzero;
  result->t_time = self->t_time / result->t_time;
 } else {
  if (object_as_time_half(other,&result->t_months))
      goto err;
  if unlikely(!result->t_months) goto err_divzero;
...
PRIVATE struct type_math time_math = {
    ...
    /* .tp_mul    = */(DREF DeeObject *(DCALL *)(DeeObject *__restrict,DeeObject *__restrict))&time_mul,
    /* .tp_div    = */(DREF DeeObject *(DCALL *)(DeeObject *__restrict,DeeObject *__restrict))&time_div,
    /* .tp_mod    = */(DREF DeeObject *(DCALL *)(DeeObject *__restrict,DeeObject *__restrict))&time_mod,
...
PRIVATE DREF DeeObject *DCALL
f_libtime_maketime(size_t argc, DeeObject **__restrict argv, DeeObject *kw) {
 DREF DeeTimeObject *result;
 ...
 if (time_setint(result,TIME_REPR_HOR,hor) ||
     ...
     goto err2;
 DeeObject_Init(result,&DeeTime_Type);
done: return (DREF DeeObject *)result;
err2: DeeObject_Free(result);
err:  return NULL;
}
PRIVATE DREF DeeObject *DCALL
f_libtime_makedate(size_t argc, DeeObject **__restrict argv, DeeObject *kw) {
 DREF DeeTimeObject *result;
 dtime_half_t yer = 0;
...
PRIVATE DREF DeeObject *DCALL
f_libtime_makeanon(size_t argc, DeeObject **__restrict argv) {
...
PRIVATE struct type_cmp time_cmp = {
    ...
    /* .tp_eq   = */(DREF DeeObject *(DCALL *)(DeeObject *__restrict self, DeeObject *__restrict some_object))&time_eq,
    /* .tp_ne   = */(DREF DeeObject *(DCALL *)(DeeObject *__restrict self, DeeObject *__restrict some_object))&time_ne,
    /* .tp_lo   = */(DREF DeeObject *(DCALL *)(DeeObject *__restrict self, DeeObject *__restrict some_object))&time_lo,
    /* .tp_le   = */(DREF DeeObject *(DCALL *)(DeeObject *__restrict self, DeeObject *__restrict some_object))&time_le,
    /* .tp_gr   = */(DREF DeeObject *(DCALL *)(DeeObject *__restrict self, DeeObject *__restrict some_object))&time_gr,
    /* .tp_ge   = */(DREF DeeObject *(DCALL *)(DeeObject *__restrict self, DeeObject *__restrict some_object))&time_ge
};
