...
parse_asm_flags(char *__restrict str,
 ...
 for (;;) {
  ...
  if (!flag_end) flag_end = strend(str);
  if (flag_start < flag_end) {
   bool remove_flag = false; size_t optlen;
   if (*flag_start == 'n') {
    ++flag_start;
    if (flag_start[0] == 'o' &&
        flag_start[1] == '-')
   ...
   if likely(optlen < COMPILER_STRLEN(disasm_flags[0].name)) {
    for (i = 0; i < COMPILER_LENOF(disasm_flags); ++i) {
     if (disasm_flags[i].name[optlen]) continue;
     if (memcmp(disasm_flags[i].name,flag_start,optlen*sizeof(char)) != 0) continue;
     /* Found it! (update the resulting set of flags) */
     if (remove_flag ^ disasm_flags[i].invert)
          *presult &= ~disasm_flags[i].flag;
     else *presult |=  disasm_flags[i].flag;
     goto next_opt;
    }
   }
...
PRIVATE struct dex_symbol symbols[] = {
    { "printcode", (DeeObject *)&libdisasm_public_printcode, MODSYM_FNORMAL,
      DOC("(code co,int flags=0)->string\n"
          "(code co,string flags=\"\")->string\n"
          "(code co,file out,int flags=0)->int\n"
          "(code co,file out,string flags=\"\")->int\n"
          "@throw ValueError The given @flags string contains an unknown flag\n"
          "@param flags Either an integer set of internal flags, or a comma-seperated options list (see below)\n"
          "Print the given code object @co, either to file @out, or into a string that is then returned\n"
          "When @flags is given as a string, it must be a comma-seperated list of the following options:\n"
          "%{table Name|Behavior\n"
