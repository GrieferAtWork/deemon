...
/*[[[deemon import("rt._dexutils").gw("lseek", "fd:unix:fd,offset:I64d,whence:d->?Dint", libname: "posix"); ]]]*/
...
PRIVATE WUNUSED DREF DeeObject *DCALL posix_lseek_f(size_t argc, DeeObject *const *argv, DeeObject *kw) {
	...
	int64_t offset;
	int whence;
	if (DeeArg_UnpackKw(argc, argv, kw, posix_kwds_fd_offset_whence, "o" UNPd64 "d:lseek", &fd, &offset, &whence))
		goto err;
	fd_fd = DeeUnixSystem_GetFD(fd);
	if unlikely(fd_fd == -1)
		goto err;
...
#if !defined(posix_lseek_IS64) || defined(__DEEMON__)
/*[[[deemon import("rt._dexutils").gw("lseek", "fd:unix:fd,offset:I32d,whence:d->?Dint", libname: "posix"); ]]]*/
...
PRIVATE WUNUSED DREF DeeObject *DCALL posix_lseek_f(size_t argc, DeeObject *const *argv, DeeObject *kw) {
	...
	int32_t offset;
	int whence;
	if (DeeArg_UnpackKw(argc, argv, kw, posix_kwds_fd_offset_whence, "o" UNPd32 "d:lseek", &fd, &offset, &whence))
		goto err;
	fd_fd = DeeUnixSystem_GetFD(fd);
	if unlikely(fd_fd == -1)
		goto err;
...
/*[[[deemon import("rt._dexutils").gw("write", "fd:unix:fd,buf:obj:buffer,count:Iud=-1->?Dint", libname: "posix"); ]]]*/
