...
PRIVATE struct type_member tpconst cellfmt_members[] = {
	TYPE_MEMBER_FIELD_DOC("__rowfmt__", STRUCT_OBJECT_AB, offsetof(CellFmt, cfo_fmt), "->?G_RowFmt"),
	TYPE_MEMBER_END
};
...
err_oob:
	DeeRT_ErrIndexOutOfBounds(Dee_AsObject(self), index, self->rfc_fmt->rf_ncol);
	return NULL;
}
...
PRIVATE struct type_member tpconst rowfmtcolumns_members[] = {
	TYPE_MEMBER_FIELD_DOC("__rowfmt__", STRUCT_OBJECT_AB,
...
err_obb:
	DeeRT_ErrIndexOutOfBounds(Dee_AsObject(self), index, self->rf_ncol);
	return NULL;
}
...
rowfmt_getitem_string_hash(RowFmt *__restrict self, char const *name, Dee_hash_t UNUSED(hash)) {
	...
	if unlikely(index == (unsigned int)-1) {
		DeeRT_ErrUnboundKeyStr(Dee_AsObject(self), name);
		return NULL;
	}
...
rowfmt_getitem_string_len_hash(RowFmt *__restrict self, char const *name, size_t namelen, Dee_hash_t UNUSED(hash)) {
	...
	if unlikely(index == (unsigned int)-2)
		return NULL;
	if unlikely(index == (unsigned int)-1) {
		DeeRT_ErrUnboundKeyStrLen(Dee_AsObject(self), name, namelen);
		return NULL;
	}
	return rowfmt_getitem_index(self, index);
}
