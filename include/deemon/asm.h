 ...
 * $ deemon -F include/deemon/asm-table.h lib/rt/bytecode.dee
...
#define ASM_THROW             0x03 /* [1][-1,+0]   `throw pop`                          - Pop one Object and throw it as an exception. This instruction does not return normally.
                                    ...
                                    * >> THROW(POP());
                                    * >> EXCEPT(); */
#define ASM_RETHROW           0x04 /* [1][-0,+0]   `throw except`                       - Rethrow the current exception (Generated by `throw` without an argument).
                                    * >> EXCEPT(); */
...
/*      ASM_                  0x0a  *               --------                            - ------------------ */
#define ASM_PUSH_BND_ARG      0x0b /* [2][-0,+1]   `push bound arg <imm8>`              - Check if the argument variable indexed by `<imm8>` is bound, pushing true/false indicative of that state.
...
#define ASM_SWAP              0x20 /* [1][-2,+2]   `swap`                               - Swap the 2 top-most stack entries.
                                    ...
                                    * >> TOP = SECOND;
                                    * >> SECOND = ob; */
#define ASM_LROT              0x21 /* [2][-3-n,+3+n] `lrot #<imm8>+3`                   - Rotate the top <imm8>+3 stack objects left by one; away from SP.
                                    * [2][-2-n,+2+n] `lrot #<imm8>+2, PREFIX`           - `PREFIX: lrot #<imm8>+3`
...
#define ASM_PUSH_ARG          0x37 /* [2][-0,+1]   `push arg <imm8>`                    - Push the argument indexed by `<imm8>`.
                                    * [2][-0,+0]   `mov  PREFIX, arg <imm8>`            - `PREFIX: push arg <imm8>`
                                    * >> DESTINATION = ARG(IMM8); */
#define ASM_PUSH_CONST        0x38 /* [2][-0,+1]   `push const <imm8>`                  - Same as `ASM_PUSH_STATIC`, but doesn't require a lock as the slot should acts as an immutable constant.
...
#define ASM_ADD_IMM32         0x82 /* [5][-1,+1]   `add top, $<imm32>`                  - Same as `ASM_ADD`, but use <imm32> as second operand.
                                    * [5][-0,+0]   `add PREFIX, $<imm32>`               - `PREFIX: add $<Simm8>` */
...
#define ASM_SUB_IMM32         0x84 /* [5][-1,+1]   `sub top, $<imm32>`                  - Same as `ASM_SUB`, but use <imm32> as second operand.
                                    * [5][-0,+0]   `sub PREFIX, $<imm32>`               - `PREFIX: sub $<Simm8>` */
...
#define ASM_SETMEMBERI_THIS_R 0xdb /* [3][-1,+0]   `setmemberi this, ref <imm8>, $<imm8>, pop` - Same as `ASM_SETMEMBER_THIS_R`, but does `DeeRT_ErrCAlreadyBoundInstanceMember()` if the member already has a value (the "i" stands for "initial"). */
...
/*      ASM_                  0xfb  *      --------                            - ------------------ */
#define ASM_EXTERN            0xfc /* [3] `extern <imm8>:<imm8>`               - Same as `ASM_LOCAL`, but used to write to extern variables. */
#define ASM_STATIC            0xfd /* [2] `static <imm8>`                      - Same as `ASM_LOCAL`, but used to write to static variables. */
#define ASM_GLOBAL            0xfe /* [2] `global <imm8>`                      - Same as `ASM_LOCAL`, but used to write to global variables. */
#define ASM_LOCAL             0xff /* [2] `local <imm8>`
...
#define ASM16_LROT            0xf021 /* [4][-3-n,+3+n] `lrot #<imm16>+3`                  - Rotate the top #<imm16>+2 stack objects left by one; away from SP.
                                      * [4][-2-n,+2+n] `lrot #<imm16>+2, PREFIX`          - `PREFIX: lrot #<imm16>+3` */
...
#define ASM_PUSH_THIS_FUNCTION 0xf035/* [2][-0,+1]   `push this_function`                 - Push the calling function onto the stack.
                                      * [2][-0,+0]   `mov  PREFIX, this_function`         - `PREFIX: push this_function` */
#define ASM16_PUSH_MODULE     0xf036 /* [4][-0,+1]   `push module <imm16>`                - Push an imported module indexed by <imm16> from the current module.
                                      * [4][-0,+0]   `mov  PREFIX, module <imm16>`        - `PREFIX: push module <imm16>` */
#define ASM16_PUSH_ARG        0xf037 /* [4][-0,+1]   `push arg <imm16>`                   - Push the argument indexed by `<imm16>`.
                                      * [4][-0,+0]   `mov  PREFIX, arg <imm16>`           - `PREFIX: push arg <imm16>` */
#define ASM16_PUSH_CONST      0xf038 /* [4][-0,+1]   `push const <imm16>`                 - Same as `ASM_PUSH_STATIC16`, but doesn't require a lock as the slot should acts as an immutable constant.
...
#define ASM16_PUSH_EXTERN     0xf03c /* [6][-0,+1]   `push extern <imm16>:<imm16>`        - Push the extern variable indexed by `<imm16>:<imm16>`. If it isn't bound, throw an `Error.RuntimeError.UnboundLocal`
                                      * [6][-0,+0]   `mov  PREFIX, extern <imm16>:<imm16>`- `PREFIX: push extern <imm16>:<imm16>` */
...
#define ASM16_UNPACK          0xf046 /* [4][-1,+n]   `unpack pop, #<imm16>`               - Pop a sequence and unpack it into <imm16> elements then pushed onto the stack.
                                      * [4][-0,+n]   `unpack PREFIX, #<imm16>`, `PREFIX: unpack #<imm16>`
...
#define ASM_PACK_DICT         0xf053 /* [3][-n*2,+1] `push pack Dict, #<imm8>*2`          - Pop <imm8>*2 elements and pack them into a Dict, using every first as key and every second as item.
...
#define ASM16_PACK_DICT       0xf063 /* [4][-n*2,+1] `push pack Dict, #<imm16>*2`         - Pop <imm16>*2 elements and pack them into a Dict, using every first as key and every second as item.
...
#define ASM16_NOP             0xf090 /* [2][-0,+0]   `nop16`                              - Same as `ASM_NOP` (we ignore the F0 prefix)
                                      * [2][-0,+0]   `nop16 PREFIX` - `PREFIX: nop16` */
/*      ASM_                  0xf091  *               --------                            - ------------------ */
/*      ASM_                  0xf092  *               --------                            - ------------------ */
/*      ASM_                  0xf093  *               --------                            - ------------------ */
...
#define ASM16_SETMEMBERI_THIS_R 0xf0db/*[6][-1,+0]   `setmemberi this, ref <imm16>, $<imm16>, pop` - Same as `ASM16_SETMEMBER_THIS_R`, but does `DeeRT_ErrCAlreadyBoundInstanceMember()` if the member already has a value (the "i" stands for "initial"). */
...
/*      ASM_                  0xf0f8  *      --------                            - ------------------ */
/*      ASM_                  0xf0f9  *      --------                            - ------------------ */
#define ASM16_STACK           0xf0fa /* [4] `stack #<imm16>`                     - Use an Object located on the stack as storage class.
                                      *                                            The associated operand is an absolute offset from the stack's base. */
/*      ASM_                  0xf0fb  *      --------                            - ------------------ */
#define ASM16_EXTERN          0xf0fc /* [6] `extern <imm16>:<imm16>`             - Same as `ASM_LOCAL16`, but used to write to extern variables. */
#define ASM16_STATIC          0xf0fd /* [4] `static <imm16>`                     - Same as `ASM_LOCAL16`, but used to write to static variables. */
#define ASM16_GLOBAL          0xf0fe /* [4] `global <imm16>`                     - Same as `ASM_LOCAL16`, but used to write to global variables. */
#define ASM16_LOCAL           0xf0ff /* [4] `local  <imm16>`                     - Same as `ASM_LOCAL`, but encodes a little-endian 16-bit local id. */
