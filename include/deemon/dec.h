...
typedef struct ATTR_PACKED {
	...
	uint64_t              e_deemon_build_id[2];  /* Deemon build ID (a random UUID generated when compiling deemon) */
	uint64_t              e_deemon_host_id[2];   /* Deemon host ID (a random UUID generated when installing deemon / starting deemon for the first time) */
	...
	union {
		...
		struct {
			Dee_dec_addr32_t er_offsetof_srel;   /* [1..1] Offset to array of `Dee_dec_addr32_t[]' (terminated by a 0-entry) of offsets where "char*"-pointers needs to be relocated via `+=(uintptr_t)DeeDec_Ehdr_GetModule(this)' */
			Dee_dec_addr32_t er_offsetof_drel;   /* [1..1] Offset to array of `Dee_dec_addr32_t[]' (terminated by a 0-entry) of offsets where "char*"-pointers needs to be relocated via `+=(uintptr_t)DeeModule_GetDeemon()' */
			Dee_dec_addr32_t er_offsetof_drrel;  /* [1..1] Same as `e_offsetof_drel', but `Dee_Incref()' during relocation */
		...
		struct {
			Dee_dec_addr32_t      *ei_drrel;     /* [0..*][owned] List of incref-relocations against deemon-core objects (terminated by `* == NULL' or `Dec_Ehdr__image__drrelcount()') */
#define Dec_Ehdr__image__drrelcount(self) (Dee_MallocUsableSize((self)->e_typedata.td_image.ei_drrel) / sizeof(Dee_dec_addr32_t))
			struct Dee_dec_depmod *ei_deps;      /* [0..*][owned] Vector of dependent modules (terminated by `->ddm_mod == NULL' or `Dec_Ehdr__image__depcount()') */
#define Dec_Ehdr__image__depcount(self)   (Dee_MallocUsableSize((self)->e_typedata.td_image.ei_deps) / sizeof(struct Dee_dec_depmod))
...
typedef struct ATTR_PACKED {
	union {
		...
		DREF DeeModuleObject *d_mod;              /* Used internally during relocation */
		uint64_t             _d_align;            /* Force alignment */
	}                d_modspec;
	Dee_dec_addr32_t d_offsetof_mrel;    /* [1..1] Offset to array of `Dee_dec_addr32_t[]' (terminated by a 0-entry) of offsets where "char*"-pointers needs to be relocated via `+=(uintptr_t)(DeeModuleObject *)DEPENDENCY' */
...
#define DeeDec_Ehdr_GetModule(self) \
	((DeeModuleObject *)DeeGC_Object((struct Dee_gc_head *)(&(self)->e_heap.hr_first + 1)))
#define DeeDec_Ehdr_FromModule(self)                                                  \
	((Dec_Ehdr *)((uintptr_t)DeeGC_Head(Dee_REQUIRES_TYPE(DeeModuleObject *, self)) - \
...
struct Dee_dec_objtab {
	struct Dee_dec_objtab_entry *dot_list; /* [0..dot_size][SORT(dote_obj ASC)][owned] List of already-encoded objects */
	size_t                       dot_size; /* Amount of non-NULL key-item pairs. */
};
