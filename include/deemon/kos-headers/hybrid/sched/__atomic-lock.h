...
struct __hybrid_atomic_lock {
	unsigned int a_lock; /* Lock word. (non-zero if held) */
};

#define __HYBRID_ATOMIC_LOCK_INIT                 { 0 }
#define __HYBRID_ATOMIC_LOCK_INIT_ACQUIRED        { 1 }
#define __hybrid_atomic_lock_init(self)           (void)((self)->a_lock = 0)
#define __hybrid_atomic_lock_init_acquired(self)  (void)((self)->a_lock = 1)
#define __hybrid_atomic_lock_cinit(self)          __hybrid_assert((self)->a_lock == 0)
#define __hybrid_atomic_lock_cinit_acquired(self) (void)((self)->a_lock = 1)

#define __hybrid_atomic_lock_acquired(self)   (__hybrid_atomic_load(&(self)->a_lock, __ATOMIC_ACQUIRE) != 0)
#define __hybrid_atomic_lock_available(self)  (__hybrid_atomic_load(&(self)->a_lock, __ATOMIC_ACQUIRE) == 0)
#define __hybrid_atomic_lock_tryacquire(self) __likely(__hybrid_atomic_xch(&(self)->a_lock, 1, __ATOMIC_ACQUIRE) == 0)
#if defined(NDEBUG) || defined(NDEBUG_SYNC)
#define __hybrid_atomic_lock_release(self) ___hybrid_atomic_lock_release_NDEBUG(self)
#else /* NDEBUG || NDEBUG_SYNC */
#define __hybrid_atomic_lock_release(self) \
	(__hybrid_assert(__hybrid_atomic_lock_acquired(self)), ___hybrid_atomic_lock_release_NDEBUG(self))
#endif /* !NDEBUG || !NDEBUG_SYNC */
#define ___hybrid_atomic_lock_release_NDEBUG(self) \
...
#define __hybrid_atomic_lock_acquire_smp_r(self, p_flag) __hybrid_preemption_acquire_smp_r(__hybrid_atomic_lock_tryacquire(self), p_flag)
#define __hybrid_atomic_lock_release_smp_r(self, p_flag) __hybrid_preemption_release_smp_r(__hybrid_atomic_lock_release(self), p_flag)
#define __hybrid_atomic_lock_acquire_smp_b(self)         __hybrid_preemption_acquire_smp_b(__hybrid_atomic_lock_tryacquire(self))
#define __hybrid_atomic_lock_release_smp_b(self)         __hybrid_preemption_release_smp_b(__hybrid_atomic_lock_release(self))
#define __hybrid_atomic_lock_acquire_smp(self)           __hybrid_preemption_acquire_smp(__hybrid_atomic_lock_tryacquire(self))
#define __hybrid_atomic_lock_release_smp(self)           __hybrid_preemption_release_smp(__hybrid_atomic_lock_release(self))
...
for (local S, N: {
	...
	print("                                                  struct __hybrid_atomic_lock *__restrict __self) {");
	print("	", T, " __result;");
	print("	__hybrid_atomic_lock_acquire(__self);");
	print("	__result = __hybrid_atomic_load", N, "(__p, __order);");
	print("	__hybrid_atomic_lock_release(__self);");
...
__NOTHROW(__hybrid_atomic_load8_with_atomic_lock)(__UINT8_TYPE__ const *__restrict __p, int __order,
                                                  struct __hybrid_atomic_lock *__restrict __self) {
	__UINT8_TYPE__ __result;
	__hybrid_atomic_lock_acquire(__self);
	__result = __hybrid_atomic_load8(__p, __order);
	__hybrid_atomic_lock_release(__self);
	return __result;
}
...
, __T>::__type __NOTHROW_NCX(__hybrid_atomic_load_with_atomic_lock)(__T const *__p, int __order, struct __hybrid_atomic_lock *__restrict __self) {
	(void)__order;
	(void)__self;
