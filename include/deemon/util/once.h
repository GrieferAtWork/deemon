...
#define Dee_once_begin(self) \
	(__hybrid_atomic_load(&(self)->oc_didrun, __ATOMIC_ACQUIRE) >= DEE_ONCE_COMPLETED_THRESHOLD ? 0 : (Dee_once_begin)(self))
#define Dee_once_begin_noint(self) \
	(__hybrid_atomic_load(&(self)->oc_didrun, __ATOMIC_ACQUIRE) >= DEE_ONCE_COMPLETED_THRESHOLD ? false : (Dee_once_begin_noint)(self))
