...
| `CCCCCCCC` | Uninitialized heap memory by `Dee_Malloc`<br/>Various instances of `DBG_memset()` to explicitly trash uninitialized memory | y | `DL_DEBUG_MEMSET_ALLOC_PATTERN` |
| `CDCDCDCD` | Uninitialized stack memory by `Dee_Malloca` | y | `Dee_MALLOCA_SKEW_ALLOCA` |
| `DEADBEEF` | Heap memory after `Dee_Free()` | y | `DL_DEBUG_MEMSET_FREE_PATTERN` |
