...
| `2`  | -      | `hr_first`            | `{0x000001C0, 0x000000C4}` | First heap chunk (see **HEAP** section on *`struct Dee_heapchunk` (Chunk)*).<br/>In regular `.dec` files, this first chunk is assumed to hold the `struct Dee_gc_head_link` of the primary `DeeModuleObject` as its payload. |
| ...  | ...    | ...                   | N/A                  | ... |
