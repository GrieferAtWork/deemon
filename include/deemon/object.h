...
#define MEMBERCACHE_COUNT  4           /* Amount of different cache types. */
    uint16_t               mcs_type;   /* The type of this slot (One of `MEMBERCACHE_*') */
    uint16_t               mcs_pad[(sizeof(void *)-sizeof(uint16_t))/sizeof(uint16_t)];
    ...
    union {
        ...
        struct type_method mcs_method; /* [valid_if(mcs_type == MEMBERCACHE_METHOD || mcs_type == MEMBERCACHE_OBMETH)] */
        struct type_getset mcs_getset; /* [valid_if(mcs_type == MEMBERCACHE_GETSET || mcs_type == MEMBERCACHE_OBPROP)] */
        struct type_member mcs_member; /* [valid_if(mcs_type == MEMBERCACHE_MEMBER || mcs_type == MEMBERCACHE_OBMEMB)] */
    };
};
...
INTERN int DCALL DeeObject_GenericFindAttrString(DeeTypeObject *__restrict tp_self, struct attribute_info *__restrict result, struct attribute_lookup_rules const *__restrict rules);
